...
// deepanshu madrchod gaandu ne code uda diya 

{
	...
	// 	$items = get_items_by_type('estimate', $estimate_id); 
...
public function get_products_by_estimate($estimate_id)
{
	...
	// Fetch tolerance and delivery_date
	$this->db->select('tolerance, delivery_date');
...
// Load initial dropdown options for all filters
public function load_filter_options() {
  $colors = $this->manufacturing_model->get_all_colors();
  $thicknesses = $this->manufacturing_model->get_all_thicknesses();
  $materials = $this->manufacturing_model->get_all_materials();
  $finishes = $this->manufacturing_model->get_all_finishes();
  $profiles = $this->manufacturing_model->get_all_profiles();
...
// Return filtered product list based on all selected attributes
public function filter_products_by_attributes() {
  ...
  $products = $this->manufacturing_model->get_products_by_filters(
    $color_id,
    $thickness,
    $material_type_id,
    $finish_type_id,
...
public function bom_export_pdf($id)
{
	...
	public function add_edit_work_center($id = '')
	{
		...
		if ($id != '') {
		    $data['machines'] = $this->manufacturing_model->get_machines_by_work_center($id);
		} else {
		    $data['machines'] = []; // empty for Add
		}
...
public function update_scrap_data() {
    ...
    public function bill_of_material_manage()
	{
		...
		$data['colors'] = $this->manufacturing_model->get_all_colors();
		$data['thicknesses'] = $this->manufacturing_model->get_all_thicknesses();
		$data['materials'] = $this->manufacturing_model->get_all_materials();
		$data['finishes'] = $this->manufacturing_model->get_all_finishes();
		$data['profiles'] = $this->manufacturing_model->get_all_profiles();
		$bom_type=[];
	...
	public function add_bill_of_material_modal($id='')
	{
		...
		if ($this->input->post()) {
			$data = $this->input->post();
			unset(
				$data['filter_color_id'],
				$data['filter_thickness'],
...
public function get_detail_by_product($product_id)
{
    if ($this->input->is_ajax_request()) {
        $this->load->model('manufacturing_model');
...
$data['oprater']=5;
	...
	public function view_work_order($id, $manufacturing_order_id)
	{
		...
		$data['product_detail'] = $this->manufacturing_model->get_product_by_id_with_finish($data['work_order']->product_id);
        // $data['product_images'] = $this->manufacturing_model->get_product_images_by_id($data['work_order']->product_id);
		$data['arr_images'] = $this->manufacturing_model->get_all_product_images_grouped();
        // Fetch Routing Operation Description
		$routing_detail_id = $data['work_order']->routing_detail_id;
		$routing_detail = $this->manufacturing_model->get_operation($routing_detail_id); 

		$data['routing_operation_description'] = $routing_detail ? $routing_detail->description : ''; 
		...
		$data['machines'] = $this->manufacturing_model->get_machines_by_work_center($data['work_order']->work_center_id);
		$data['machine_splits'] = $this->manufacturing_model->get_work_order_machine_splits($id);
	 ...
	 */
	/*
	public function mo_mark_as_start_working()
	{
		if (!$this->input->is_ajax_request()) {
			show_404();
		}
		...
		$mo_id = $data['manufacturing_order'];
		...
		$this->manufacturing_model->update_manufacturing_order_status($mo_id, ['status' => 'in_progress']);
		
		// Optional: Fetch capacity from machine
		
		// $machine = $this->db->get_where(db_prefix().'mrp_machines', ['id' => $machine_id])->row();
		// $capacity = $machine ? $machine->capacity_per_hour : 0;

		// // Insert tracking
		// $insert = $this->db->insert(db_prefix() . 'mrp_work_order_machine_trackings', [
		// 	'work_order_id' => $work_order_id,
		// 	'machine_id' => $machine_id,
		// 	'from_date' => $current_time,
		// 	'staff_id' => get_staff_user_id(),
		// 	'produced_quantity' => $data['produced_quantity'],
		// 	'total_assigned_quantity' => 0, // set if known
	...
	*/
	...
	public function start_split_task($machine_tracking_id)
	{
	    if (!$this->input->is_ajax_request()) {
	        show_404();
	    }
	    ...
	    $split = $this->manufacturing_model->get_machine_tracking_by_id($machine_tracking_id);
	    if (!$split) {
	        echo json_encode(['status' => 'warning', 'message' => 'Invalid task tracking ID']);
	        return;
	    }

	    // Security: allow only the assigned operator
	    $current_user_id = get_staff_user_id();
		$is_admin = is_admin();
		$is_manager = has_permission('manufacturing', '', 'edit'); // assuming manager has 'edit' permission

		if ($split['staff_id'] != $current_user_id && !$is_admin && !$is_manager) {
			echo json_encode(['status' => 'warning', 'message' => 'You are not the assigned operator, admin, or manager.']);
			return;
		}

	    $current_time = date('Y-m-d H:i:s');

	    // Update the tracking start
	    $updated = $this->manufacturing_model->update_machine_tracking_start($machine_tracking_id, [
	        'from_date' => $current_time,
	    ]);

	    // Optionally update work_order and MO status if not already in_progress
	    $this->manufacturing_model->update_work_order_status($split['work_order_id'], ['status' => 'in_progress']);
	    //$this->manufacturing_model->update_manufacturing_order_status_by_work_order($split['work_order_id'], 'in_progress');

	    if ($updated) {
	        echo json_encode(['status' => 'success', 'message' => _l('Task started successfully')]);
	    } else {
	        echo json_encode(['status' => 'warning', 'message' => _l('Start failed or already started')]);
	    }
	}
	...
	public function get_machines_by_work_center($id)
	{
		$machines = $this->manufacturing_model->get_machines_by_work_center($id);
		$staffs = $this->staff_model->get();
		//print_r($machines);
		foreach ($machines as &$machine) {
			$options = '';
			foreach ($staffs as $staff) {
				$selected = $machine->operator_id == $staff['staffid'] ? 'selected' : '';
				$options .= '<option value="' . $staff['staffid'] . '" ' . $selected . '>' . get_staff_full_name($staff['staffid']) . '</option>';
			}
			$machine->staff_options_html = $options;
		}
	...
	public function save_split_assignments()
	{
	    if (!$this->input->is_ajax_request()) {
	        show_404();
	    }
	    ...
	    $work_order_id = $this->input->post('work_order_id');
		$work_center_id = $this->input->post('work_center_id');
		$product_id = $this->input->post('product_id');

		$machine_ids = $this->input->post('machine_id');
		$staff_ids = $this->input->post('staff_id');
		$quantities = $this->input->post('assigned_quantity');
		...
		if (!empty($machine_ids) && is_array($machine_ids)) {
		    for ($i = 0; $i < count($machine_ids); $i++) {
		        $splits[] = [
		            ...
		            'total_assigned_quantity' => (int)$quantities[$i],
		            'from_date' => date('Y-m-d H:i:s'),
		            'produced_quantity' => 0,
		            'machine_capacity' => 0, // Optional: calculate if needed
		        ];
		    }
		}
	    ...
	    foreach ($splits as $split) {
	        $total_quantity += (int)$split['assigned_quantity'];
	    }

	    // Fetch max allowable quantity from WO (assume you have this function)
	    $product_info = $this->manufacturing_model->get_product_by_id_with_finish($this->input->post('product_id'));

		$max_quantity = 0;
		if ($product_info && isset($product_info->quantity)) {
		    $max_quantity = (int) $product_info->quantity;
		}

	    if ($total_quantity > $max_quantity) {
	        echo json_encode(['status' => 'error', 'message' => 'Total quantity exceeds work order quantity']);
	        return;
	    }

	    foreach ($splits as $split) {
		    $this->db->insert(db_prefix() . 'mrp_work_order_machine_trackings', $split);
		}

	    echo json_encode(['status' => 'success', 'message' => 'Machines split saved successfully']);
	}
