	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				...
				$machine_array = ['']; // default: one empty field
				...
				if(isset($operation)){
				   ...
				   $staff_id_selected = $operation->staff_id;
				   /*
					if (isset($operation) && isset($operation->machine_names) && !empty($operation->machine_names)) {
					    $machine_array = explode(',', $operation->machine_names);
					}
					*/
					...
					<div class="row">
						...
						<div class="row">
							<div class=col-md-12>
								...
								<div class="col-md-6">
											<?php echo render_select('staff_id', $staffs, array('staffid', array('firstname', 'lastname')), 'Operation Manager (Responsible)', $staff_id_selected, [], [], '', '', false); ?>
								</div>
								<div class="col-md-6">
								...
								<div class="col-md-12">
									...
									echo render_textarea('description','',(isset($operation) ? $operation->description : ''),array('rows'=>6,'placeholder'=>_l('task_add_description'),'data-task-ae-editor'=>true, !is_mobile() ? 'onclick' : 'onfocus'=>(!isset($operation) || isset($operation) && $operation->description == '' ? '' : '' )),array(),'','tinymce-operation'); ?>
								</div>
							</div>
						...
						/*
						<div class="col-md-12">
						    <label class="control-label"><?php echo _l('machine_name'); ?></label>
						    <div class="machine-group">
						        <?php foreach ($machine_array as $index => $machine) { ?>
						            <div class="input-group m-b-10">
						                <input type="text" name="machine_name[]" class="form-control" placeholder="<?php echo _l('enter_machine_name'); ?>" value="<?php echo htmlspecialchars(trim($machine)); ?>">
						                <span class="input-group-btn">
						                    <?php if ($index === 0) { ?>
						                        <button class="btn btn-success add-machine-btn" type="button"><i class="fa fa-plus"></i></button>
						                    <?php } else { ?>
						                        <button class="btn btn-danger remove-machine-btn" type="button"><i class="fa fa-times"></i></button>
						                    <?php } ?>
						                </span>
						            </div>
						        <?php } ?>
								...
								<div id="contract_attachments" class="mtop30 ">
									<?php if(isset($operation_attachment)){ ?>
										...
										foreach($operation_attachment as $attachment) {
											...
											if( has_permission('manufacturing', '', 'delete')&&has_permission('routing', '', 'delete')){
