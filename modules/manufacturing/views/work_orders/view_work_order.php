...
<style>
	...
	.fullscreen-modal img.wo-modal-content {
		max-height: 80vh;
		max-width: 90vw;
		object-fit: contain;
		display: block;
...
<div id="wrapper">
    ...
    <?php if (has_permission('manufacturing', '', 'create') && has_permission('work_order', '', 'operate_assinged') ||has_permission('work_order', '', 'operate_global')
        ...
        <?php if (!$check_mo_cancelled) { ?>
			...
			*/
			?>
			<?php if (empty($machine_splits)) { ?>
			<button type="button"
				class="btn btn-sm btn-<?php echo new_html_entity_decode($start_working_status); ?> pull-left open_split_modal mright5"
				data-work-order-id="<?php echo $work_order_id; ?>"
				data-work-center-id="<?php echo $work_center; ?>"
				data-max-qty="<?php echo $qty_producing; ?>"
				data-product-id="<?php echo $product_id; ?>"
			>
			<?php }	?>
				<?php echo _l('split_and_start_working'); ?>
			</button>
            <div class="<?php echo new_html_entity_decode($action_hide) ?>">
                <button type="button" class="btn btn-warning pull-left mark_pause mright5"><?php echo _l('mrp_pause'); ?></button>
                <button type="button" class="btn btn-success pull-left mark_done mright5"><?php echo _l('mrp_done'); ?></button>
...
<div class="row">
    ...
    <thead >
        <tr  style="background-color: #343a40; color: #ffffff;">
            ...
            <th class="text-center">Color <br><hr style="margin: 2px 0;">Finish</th>
            <th class="text-center">Finish Side</th>
            <th class="text-center">Thickness</th>
            <th class="text-center">Qty</th>
            <th class="text-center">Length</th>
            <th class="text-center">Size</th>
            <th class="text-center">Bend Type</th>
			<th class="text-center">Development Length</th>
			<th class="text-center">No of Groove</th>

            <th class="text-center">Grooving Position</th>
        </tr>
    </thead>
    <tbody>
        <!-- Example row -->
        <tr>
            <td>1</td>
            <td width=100px><?php echo mrp_get_product_name($product_id) ; ?></td>
            <td><?php
$image_html = '<img class="images_w_table clickable-image" src="' . site_url('modules/warehouse/uploads/nul_image.jpg') . '" alt="no image" data-full="' . site_url('modules/warehouse/uploads/nul_image.jpg') . '">';

if (isset($arr_images[$product_id][0])) {
    $image = $arr_images[$product_id][0];
    $rel_id = $image['rel_id'];
    $file_name = $image['file_name'];

    $paths = [
        'modules/warehouse/uploads/item_img/' . $rel_id . '/' . $file_name,
        'modules/purchase/uploads/item_img/' . $rel_id . '/' . $file_name,
        'modules/manufacturing/uploads/products/' . $rel_id . '/' . $file_name,
    ];

    foreach ($paths as $path) {
        if (file_exists(FCPATH . $path)) {
            $image_url = site_url($path);
            $image_html = '<img class="images_w_table clickable-image" src="' . $image_url . '" alt="' . $file_name . '" data-full="' . $image_url . '">';
            break;
        }
    }
}
...
?>
            ...
            <td>
                <div><?php echo isset($product_detail->color_name) ? $product_detail->color_name : 'N/A'; ?>
</div>
                <hr style="margin: 2px 0;"> 
                <div></strong> <?php echo isset($product_detail->finish_name) ? $product_detail->finish_name : 'N/A'; ?>
</div>
            </td>
            <td>Outside</td>
            <td><?php echo isset($product_detail->Thickness) ? $product_detail->Thickness : 'N/A'; ?></td>
            <td><?php echo new_html_entity_decode($qty_production)?></td>
            <td><?php echo isset($product_detail->Length) ? $product_detail->Length : 'N/A'; ?></td>
            <td> <?php echo isset($product_detail->size_name) ? $product_detail->size_name : 'N/A'; ?></td>
            <td> <?php echo isset($product_detail->bend_type) ? $product_detail->bend_type : 'N/A'; ?></td>
			<td> <?php echo isset($product_detail->development_length) ? $product_detail->development_length : 'N/A'; ?></td>
			<td> <?php echo isset($product_detail->no_of_groove) ? $product_detail->no_of_groove : 'N/A'; ?></td>

			<td> <?php echo isset($product_detail->grooving_position) ? $product_detail->grooving_position : 'N/A'; ?></td>
...
<?php /* if (!empty($machines)) { ?>
	<div class="col-md-12">
	    <h4 class="h4-color"><?php echo _l('work_center_machines'); ?></h4>
	    <hr class="hr-color">
	    ...
	    <div class="table-responsive">
	        <table class="table table-bordered table-striped">
	            <thead>
	                <tr>
	                    <th><?php echo _l('machine_name'); ?></th>
	                    <th><?php echo _l('machine_code'); ?></th>
	                    <th><?php echo _l('assigned_operator'); ?></th>
	                    <th><?php echo _l('capacity'); ?></th>
	                    <th><?php echo _l('oee_target'); ?></th>
	                    <th><?php echo _l('costs_hour'); ?></th>
	            ...
	            <tbody>
	                <?php foreach ($machines as $machine) { ?>
	                    <tr>
	                        <td><?php echo html_escape($machine->name); ?></td>
	                        <td><?php echo html_escape($machine->code); ?></td>
	                        <td>
	                            <?php
	                            $staff_name = '';
	                            if (!empty($machine->operator_id)) {
	                                $staff = get_staff_full_name($machine->operator_id);
	                                $staff_name = $staff ? $staff : '-';
	                            }
	                            echo $staff_name;
	                            ?>
	                        </td>
	                        <td><?php echo html_escape($machine->capacity_per_hour); ?></td>
	                        <td><?php echo html_escape($machine->oee_target); ?></td>
	                        <td><?php echo html_escape($machine->cost_per_hour); ?></td>
							<td>
								<?php if (!$is_assigned_operator) { ?>
								<button type="button" data-machine-id="<?php echo $machine->id;?>" data-work-order-id="<?php echo $work_order->id;?>" data-mo-id="<?php echo $manufacturing_order_id;?>" class="btn btn-sm btn-<?php echo new_html_entity_decode($start_working_status); ?> pull-left mark_start_working mright5"
									<?php if (!$is_assigned_operator) { ?>
										onclick="alert('Only the assigned operator (<?php echo $operator_name; ?>) can start this work order!'); return false;"
									<?php } else { ?>
...
<?php } */ ?>
<?php if (!empty($machine_splits)) { ?>
    <h4><?php echo _l('Machine Splits'); ?></h4>
    <div class="table-responsive">
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th><?php echo _l('Machine'); ?></th>
                    <th><?php echo _l('Operator'); ?></th>
                    <th><?php echo _l('Assigned Quantity'); ?></th>
                    <th><?php echo _l('action');?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($machine_splits as $split) { ?>
                    <tr>
                        <td><?php echo $split['machine_name']; ?></td>
                        <td><?php echo $split['staff_name']; ?></td>
                        <td><?php echo $split['total_assigned_quantity']; ?></td>
                        <td>
							<?php if (!$is_assigned_operator) { ?>
								<button 
								    type="button" 
								    class="btn btn-sm btn-success mark_start_working_split mright5"
								    data-tracking-id="<?php echo $split['id']; ?>" 
								>
								    <?php echo _l('Start Working'); ?>
								</button>
							<?php } ?>
						</td>
                    </tr>
								...
								<div class="row">
									...
									<div class="tab-content active">
										<div role="tabpanel" class="tab-pane " id="work_instruction">
											<div class="row">
											<?php if (!empty($routing_operation_description)) : ?>
											    <div class="form-group">
											        <label><strong>Routing Operation Description:</strong></label>
											        <p><?php echo nl2br(htmlspecialchars($routing_operation_description)); ?></p>
											    </div>
											<?php endif; ?>
											</div>
											<div class="row">
...
onclick="updateScrapData()">
		...
		<div id="fullscreen-modal" class="fullscreen-modal">
			...
			<img class="wo-modal-content" id="modal-image">
		</div>
		<div class="modal fade" id="startTimerModal" tabindex="-1" role="dialog">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
			<?php echo form_open_multipart('#', array('id' => 'startTimerForm')); ?>
				<div class="modal-header">
				<h4 class="modal-title"><?php echo _l('start_working'); ?></h4>
				</div>
				<div class="modal-body">
				<input type="hidden" name="work_order_id">
				<input type="hidden" name="manufacturing_order">
				<input type="hidden" name="machine_id">											
				<div class="form-group">
					<label for="produced_quantity"><?php echo _l('produced_quantity'); ?></label>
					<input type="number" name="produced_quantity" class="form-control" min="1" required>
				</div>
				</div>
				<div class="modal-footer">
				<button type="submit" class="btn btn-primary"><?php echo _l('start'); ?></button>
		...
		?>
		<div class="modal fade" id="splitWorkOrderModal" tabindex="-1" role="dialog">
			<div class="modal-dialog modal-lg" role="document">
				<div class="modal-content">
				<?php echo form_open_multipart('#', array('id' =>'splitWorkForm','autocomplete'=>'off')); ?>
					<div class="modal-header">
					<h4 class="modal-title"><?php echo _l('split_work_order'); ?></h4>
					</div>
					<div class="modal-body">
					<input type="hidden" name="work_order_id" />
					<input type="hidden" name="work_center_id" />
					<input type="hidden" name="product_id" />
					<div class="alert alert-info">
						<strong>Total Quantity to Split:</strong> <span id="max-split-qty">0</span> Pieces
					</div>

					<div class="text-right mtop10 mleft10 mbot10">
						<button type="button" class="btn btn-default btn-sm" id="splitEquallyBtn">
							<?php echo _l('split_equally'); ?>
						</button>
					</div>

					<div id="machineOperatorList">
						<!-- dynamically populated -->
					</div>
					</div>
					<div class="modal-footer">
					<button type="submit" class="btn btn-primary"><?php echo _l('assign_and_save'); ?></button>
					<button type="button" class="btn btn-default" data-dismiss="modal"><?php echo _l('close'); ?></button>
					</div>
				</form>
				</div>
