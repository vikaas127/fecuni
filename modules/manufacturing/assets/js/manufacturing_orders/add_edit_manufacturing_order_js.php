<script>
	...
	$('select[name="product_id"]').on('change', function() {
		...
		const estimateId = $('select[name="estimate_id"]').val();
		let selectedQty = '';

		if (estimateId && currentEstimateItems.length > 0) {
			const selectedItem = currentEstimateItems.find(item => item.item_id == product_id);
			if (selectedItem) {
			selectedQty = selectedItem.qty;
			$("input[name='product_qty']").val(selectedQty);
			} else {
		...
		$.get(admin_url + 'manufacturing/get_data_create_manufacturing_order/' + product_id, function(response) {
          ...
          if (response.scrap_arr) {
			...
			const bom_id = $('select[name="bom_id"]').val();
			const qty = parseFloat($("input[name='product_qty']").val()) || 0;
			updateMOQtyDependentFields(qty, product_id, bom_id);
...
function updateMOQtyDependentFields(product_qty, product_id, bill_of_material_id) {
  if (!bill_of_material_id || !product_id) return;

  $.get(admin_url + 'manufacturing/get_bill_of_material_detail/' + bill_of_material_id + '/' + product_id + '/' + product_qty, function (response) {
      // Multiply component quantities by product_qty
      let adjustedComponents = response.component_arr.map(component => {
          component.quantity = (component.base_quantity || component.quantity) * product_qty;
          return component;
      });

      product_tabs.updateSettings({ data: adjustedComponents });

      if (typeof scrap_tabs !== 'undefined' && scrap_tabs) {
          let updatedScrapArr = response.scrap_arr.map(scrap => {
              scrap.estimated_quantity = scrap.estimated_quantity * product_qty;
              return scrap;
          });

          scrap_tabs.updateSettings({ data: updatedScrapArr });
      }
...
// Utility function
    ...
    "use strict";
		...
		const deliveryDateStr = $("input[name='estimate_delivery_date']").val();
		const deadlineStr = $("input[name='date_deadline']").val();
		const planFromStr = $("input[name='date_plan_from']").val();

		const deliveryDate = deliveryDateStr ? parseDate(deliveryDateStr) : null;
		const deadline = deadlineStr ? parseDate(deadlineStr) : null;
		const planFrom = planFromStr ? parseDate(planFromStr) : null;

		let isValid = true;
		let errorMessages = [];

		if (deliveryDate && deadline && deadline > deliveryDate) {
			isValid = false;
			errorMessages.push("❌ Deadline date cannot be after the Delivery Date.");
		}

		if (planFrom && deadline && planFrom > deadline) {
			isValid = false;
			errorMessages.push("❌ Plan From date cannot be after the Deadline date.");
		}
...
function updateProductDropdown(estimateItems) {
  ...
  const item = estimateItems[0];
  console.log('Auto-selecting single product:', item);

  $productSelect.val(item.item_id);
  $("input[name='product_qty']").val(item.qty); // 💡 Set qty directly
  $productSelect.trigger('change'); // Still call this to populate other fields
}
...
function filterProductsByEstimate(estimateId) {
  $.get(admin_url + 'manufacturing/get_products_by_estimate/' + estimateId, function (response) {
    ...
    console.log('Fetched estimateItems:', estimateItems);
	$("input[name='tolerance']").val(response.tolerance );
	if (response.delivery_date) {
		$("input[name='estimate_delivery_date']").val(response.delivery_date);
		} else {
		$("input[name='estimate_delivery_date']").val('');
	}
...
$(document).ready(function () {
  const selectedEstimateId = "<?= isset($estimate_id) ? $estimate_id : '' ?>";
  if (selectedEstimateId) {
    filterProductsByEstimate(selectedEstimateId); // This will set delivery date, tolerance, etc.
  }
});
...
<script>
$(document).ready(function () {
  let productsFromEstimate = []; // Store products loaded via estimate
  ...
  $('#estimate_id').on('change', function () {
    ...
    // Reset filters
    $('select[name="filter_color_id"], select[name="filter_thickness"], select[name="material_type_id"], select[name="finish_type_id"], select[name="profile_type_id"]').val('').selectpicker('refresh');

    if (!estimateId) {
      // No estimate selected → fallback to full product filtering
      productsFromEstimate = [];
      filterProductsByAttributes(); // use existing logic
      return;
    }

    // Fetch estimate-specific products
    $.get(admin_url + 'manufacturing/get_products_by_estimate/' + estimateId, function (response) {
      response = JSON.parse(response);
      productsFromEstimate = response.products || [];

      // Populate full product list from estimate
      populateSelect('product_id', productsFromEstimate);
    });
  });

  // Trigger product filter on any filter change
  $('#add_update_manufacturing_order').on('change', 'select[name="filter_color_id"], select[name="filter_thickness"], select[name="material_type_id"], select[name="finish_type_id"], select[name="profile_type_id"]', function () {
    const estimateId = $('#estimate_id').val();

    if (estimateId && productsFromEstimate.length > 0) {
      // Frontend filtering for estimate-specific products
      ...
      const filtered = productsFromEstimate.filter(p => {
        return (!color_id || p.color_id == color_id)
            && (!thickness || p.thickness == thickness)
            && (!material_type_id || p.material_type_id == material_type_id)
            && (!finish_type_id || p.finish_type_id == finish_type_id)
            && (!profile_type_id || p.profile_type_id == profile_type_id);
      });

      populateSelect('product_id', filtered);
    } else {
      // Normal behavior when no estimate selected
  ...
  function populateSelect(name, items) {
    ...
    // Use { value, label } or fallback to { id, description }
  ...
  // Filter products via controller (no estimate case)
  function filterProductsByAttributes() {
    ...
    console.log("Filtering products with:", { color_id, thickness, material_type_id, finish_type_id, profile_type_id });
