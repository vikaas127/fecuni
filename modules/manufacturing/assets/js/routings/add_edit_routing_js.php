<script>
	...
	function routing_init_editor(selector, settings) {
		...
		_editor_selector_check.css('visibility', 'visible');
    ...
    var _settings = {
    	...
    	setup: function(ed) {
            ...
            ed.on('init', function() {
				$(ed.getContainer()).css('visibility', 'visible');
...
$('textarea[name="description"]').css('visibility', 'visible');
$(function () {
    // Add new machine input
    $('.add-machine-btn').on('click', function () {
      var machineInput = `
        <div class="input-group m-b-10">
          <input type="text" name="machine_name[]" class="form-control" placeholder="<?php echo _l('enter_machine_name'); ?>">
          <span class="input-group-btn">
            <button class="btn btn-danger remove-machine-btn" type="button"><i class="fa fa-times"></i></button>
          </span>
        </div>`;
      $('.machine-group').append(machineInput);
    });

    // Remove machine input
