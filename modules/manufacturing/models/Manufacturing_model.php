...
// Load available colors based on joined ware_color
public function get_all_colors() {
  $this->db->select('wc.color_id as value, wc.color_name as label');
  $this->db->from('tblware_color wc');
  $this->db->join('tblitems ti', 'ti.color = wc.color_id');
  $this->db->group_by('wc.color_id');
  return $this->db->get()->result_array();
}
public function get_all_profiles() {
  $this->db->select('pt.profile_type_id as value, pt.profile_name as label');
  $this->db->from('tblmrp_profile pt');
  $this->db->join('tblitems ti', 'ti.profile_type_id = pt.profile_type_id');
  $this->db->group_by('pt.profile_type_id');
  return $this->db->get()->result_array();
}
public function get_all_materials() {
  $this->db->select('mt.material_type_id as value, mt.material_name as label');
  $this->db->from('tblmrp_material mt');
  $this->db->join('tblitems ti', 'ti.material_type_id = mt.material_type_id');
  $this->db->group_by('mt.material_type_id');
  return $this->db->get()->result_array();
}
public function get_all_finishes() {
  $this->db->select('ft.finish_type_id as value, ft.finish_name as label');
  $this->db->from('tblmrp_finish ft');
  $this->db->join('tblitems ti', 'ti.finish_type_id = ft.finish_type_id');
  $this->db->group_by('ft.finish_type_id');
  return $this->db->get()->result_array();
}


// Load unique Thickness values (capital T) from tblitems
public function get_all_thicknesses() {
  $this->db->select('ti.Thickness as value, ti.Thickness as label');
  $this->db->from('tblitems ti');
  $this->db->where('ti.Thickness !=', '');
  $this->db->group_by('ti.Thickness');
  return $this->db->get()->result_array();
}

// Filter products based on selected color and thickness
public function get_products_by_filters($color_id, $thickness, $material_type_id = null, $finish_type_id = null, $profile_type_id = null) {
  ...
  if (!empty($thickness)) {
    // Use TRIM to ensure comparison consistency
    $this->db->where('TRIM(Thickness) =', trim($thickness), false);
  }

  if (!empty($material_type_id)) {
    $this->db->where('material_type_id', $material_type_id);
  }

  if (!empty($finish_type_id)) {
    $this->db->where('finish_type_id', $finish_type_id);
  }

  if (!empty($profile_type_id)) {
    $this->db->where('profile_type_id', $profile_type_id);
  }
  ...
  return $this->db->get()->result_array();
	...
	{
		// Extract machine fields first
	// 	 print_r($data);
	// die();
    $machine_names        = $data['machine_name'] ?? [];
    $machine_codes        = $data['machine_code'] ?? [];
    $operator_ids         = $data['staff_id'] ?? [];
    $capacities           = $data['capacity'] ?? [];
    $oee_targets          = $data['oee_target'] ?? [];
    $costs_hour           = $data['costs_hour'] ?? [];
    $statuses 						= $data['status'] ?? [];
   
    // Remove machine fields from $data before insert
    unset($data['machine_name'],$data['machine_code'], $data['staff_id'], $data['capacity'], $data['oee_target'],
          $data['costs_hour'], $data['last_maintenance_at'], $data['next_maintenance_at'], $data['description'],$data['status']);
	    ...
	    for ($i = 0; $i < count($machine_names); $i++) {
	        if (empty(trim($machine_names[$i]))) continue;

	        $machine_data = [
	            'work_centre_id'      => $insert_id,
	            'operator_id'         => $operator_ids[$i],
	            'name'                => $machine_names[$i],
	            'code'                => $machine_codes[$i], // Unique machine code
	            'capacity_per_hour'   => $capacities[$i] ?? 1,
	            'oee_target'          => $oee_targets[$i] ?? 90,
	            'cost_per_hour'       => $costs_hour[$i] ?? 0,
	            'status'              => $statuses[$i] ?? 'active',
	        ];

	        $this->db->insert(db_prefix() . 'mrp_machines', $machine_data);
	    }
			return $insert_id;
		}
		return false;
	...
	public function get_machines_by_work_center($work_centre_id)
	{
	    return $this->db
	        ->where('work_centre_id', $work_centre_id)
	        ->get(db_prefix() . 'mrp_machines')
	        ->result();
	}
	...
	public function update_work_center($data, $id)
	{
		...
		$this->db->where('work_centre_id', $id);
		$this->db->delete(db_prefix() . 'mrp_machines');

		// Step 5: Re-insert updated machine list
		for ($i = 0; $i < count($machine_names); $i++) {
			if (empty(trim($machine_names[$i]))) continue;

			$machine_data = [
				'work_centre_id'      => $id,
				'operator_id'         => $operator_ids[$i] ?? null,
				'name'                => $machine_names[$i],
				'code'                => $machine_codes[$i], // you may change this to static if needed
				'capacity_per_hour'   => $capacities[$i] ?? 1,
				'oee_target'          => $oee_targets[$i] ?? 90,
				'cost_per_hour'       => $costs_hour[$i] ?? 0,
				'status'              => $statuses[$i] ?? 'active',
	...
	{
		// Delete all machines linked to this work center
    $this->db->where('work_centre_id', $id);
    $this->db->delete(db_prefix() . 'mrp_machines');
		// Then delete the work center
    $this->db->where('id', $id);
	...
	public function get_item_with_finish($item_id)
{
    $this->db->select('items.*, mrp_finish.finish_name');
    $this->db->from('items');
    $this->db->join('mrp_finish', 'items.finish_type_id = mrp_finish.id', 'left');
    $this->db->where('items.id', $item_id);
    
    $query = $this->db->get();
    return $query->row(); // use ->result() if expecting multiple
}


public function get_product_by_id_with_finish($product_id) {
    $this->db->select('items.*, mrp_finish.finish_name, ware_size_type.size_name, ware_color.color_name');
    $this->db->from(db_prefix() . 'items AS items');

    // Join with mrp_finish
    $this->db->join(db_prefix() . 'mrp_finish AS mrp_finish', 'items.finish_type_id = mrp_finish.finish_type_id', 'left');

    // Join with ware_size_type
    $this->db->join(db_prefix() . 'ware_size_type AS ware_size_type', 'items.size_id = ware_size_type.size_type_id', 'left');

    // Join with ware_color
    $this->db->join(db_prefix() . 'ware_color AS ware_color', 'items.color = ware_color.color_id', 'left');
...
public function get_product_images_by_id($product_id) {
    ...
    $this->db->where('rel_id', $product_id);

    $item_atts = $this->db->get(db_prefix() . 'files')->result_array();

    $arr_images = [];
    foreach ($item_atts as $value) {
        $arr_images[] = $value;
    }

    return $arr_images;
}
public function get_all_product_images_grouped() {
    $this->db->where('rel_type', 'commodity_item_file');
    $images = $this->db->get(db_prefix() . 'files')->result_array();

    $arr_images = [];
    foreach ($images as $img) {
        $arr_images[$img['rel_id']][] = $img;
    }
	...
	public function add_operation($data)
	{
	    ...
	    if (isset($data['machine_name']) && is_array($data['machine_name'])) {
        // Clean and implode as comma-separated string
        $machine_names = array_filter(array_map('trim', $data['machine_name']));
        $data['machine_names'] = implode(',', $machine_names);
        unset($data['machine_name']); // avoid conflict with unknown column
    	}
	    if(isset($data['file'])){
	    	unset($data['file']);
	    }
	...
	public function update_operation($data, $id)
	{
	    ...
	    if (isset($data['machine_name']) && is_array($data['machine_name'])) {
	        // Clean and convert array to comma-separated string
	        $machine_names = array_filter(array_map('trim', $data['machine_name']));
	        $data['machine_names'] = implode(',', $machine_names);
	        unset($data['machine_name']); // remove raw array before DB update
	    }
...
{
    if (is_numeric($id)) {
        $this->db->select(db_prefix() . 'items.*, ware_color.color_name');
        $this->db->from(db_prefix() . 'items');
        $this->db->join('ware_color', 'ware_color.color_id = ' . db_prefix() . 'items.color', 'left');
...
public function get_manufacturing_order_costing($id)
    ...
    public function get_machine_details($machine_id)
		{
		    $this->db->where('id', $machine_id);
		    return $this->db->get(db_prefix() . 'mrp_machines')->row();
		}

		public function get_work_order_quantity($work_order_id)
		{
		    $this->db->select('quantity');
		    $this->db->where('id', $work_order_id);
		    $result = $this->db->get(db_prefix() . 'mrp_work_orders')->row();
		...
		public function get_work_order_machine_splits($work_order_id) {
		    $this->db->select('womt.*, m.name as machine_name, CONCAT(s.firstname, " ", s.lastname) as staff_name');
		    $this->db->from(db_prefix() . 'mrp_work_order_machine_trackings womt');
		    $this->db->join(db_prefix() . 'mrp_machines m', 'm.id = womt.machine_id', 'left');
		    $this->db->join(db_prefix() . 'staff s', 's.staffid = womt.staff_id', 'left');
		    $this->db->where('womt.work_order_id', $work_order_id);
		    return $this->db->get()->result_array();
		}

		public function get_machine_tracking_by_id($id) {
		    return $this->db->get_where(db_prefix().'mrp_work_order_machine_trackings', ['id' => $id])->row_array();
		}

		public function update_machine_tracking_start($id, $data) {
		    $this->db->where('id', $id);
		    $this->db->update(db_prefix().'mrp_work_order_machine_trackings', $data);
