...
$aColumns = [
	...
	db_prefix() . 'items.id',
       'sku_code',
 'description',
// 'CONCAT(' . db_prefix() . 'items.description, " | ", ' . db_prefix() . 'items.Length, " x ", ' . db_prefix() . 'items.Width, " x ", ' . db_prefix() . 'items.Thickness," | ",' . db_prefix() . 'ware_color.color_name) as full_description',
...
if (!empty($warehouse_ft) && !is_array($warehouse_ft)) {
    $warehouse_ft = [$warehouse_ft];
}
...
$length_filter = $this->ci->input->post('length_filter');
$color_name_filter = $this->ci->input->post('color_name_filter');
...
$join = [
	...
	'LEFT JOIN ' . db_prefix() . 'mrp_finish ON ' . db_prefix() . 'mrp_finish.finish_type_id = ' . db_prefix() . 'items.finish_type_id',

		'LEFT JOIN ' . db_prefix() . 'mrp_material ON ' . db_prefix() . 'mrp_material.material_type_id = ' . db_prefix() . 'items.material_type_id',
	'LEFT JOIN ' . db_prefix() . 'mrp_profile ON ' . db_prefix() . 'mrp_profile.profile_type_id = ' . db_prefix() . 'items.profile_type_id',
...
if (!empty($warehouse_ft)) {
    ...
    // log_message('debug', 'Selected warehouse_ft: ' . $warehouse_ft);
    // log_message('debug', 'Commodity IDs found: ' . implode(', ', $arr_commodity_id));

    if (!empty($arr_commodity_id)) {
        $where[] = 'AND ' . db_prefix() . 'items.id IN (' . implode(', ', $arr_commodity_id) . ')';
    } else {
        // To ensure no results are returned if warehouse has no products
        $where[] = 'AND 1=0';
        // log_message('debug', 'No items found for selected warehouse.');
    }
}
...
if (isset($thickness_filter)) {
    $where_thickness_ft = '';
    foreach ($thickness_filter as $thickness) {
        if ($thickness != '') {
            if ($where_thickness_ft == '') {
                $where_thickness_ft .= 'AND (' . db_prefix() . 'items.Thickness = "' . $thickness . '"';
            } else {
                $where_thickness_ft .= ' OR ' . db_prefix() . 'items.Thickness = "' . $thickness . '"';
            }
        }
    }
    if ($where_thickness_ft != '') {
        $where_thickness_ft .= ')';
        array_push($where, $where_thickness_ft);
    }
}
if (isset($length_filter)) {
    $where_length_ft = '';
    foreach ($length_filter as $length) {
        if ($length != '') {
            if ($where_length_ft == '') {
                $where_length_ft .= 'AND (' . db_prefix() . 'items.Length = "' . $length . '"';
            } else {
                $where_length_ft .= ' OR ' . db_prefix() . 'items.Length = "' . $length . '"';
            }
        }
    }
    if ($where_length_ft != '') {
        $where_length_ft .= ')';
        array_push($where, $where_length_ft);
    }
}
...
if (isset($color_name_filter)) {
	$where_color_filter = '';
	foreach ($color_name_filter as $color) {
		if ($color != '') {
			if ($where_color_filter == '') {
				$where_color_filter .= 'AND ('.db_prefix().'ware_color.color_name = "' . $color . '"';
			} else {
				$where_color_filter .= ' Or'.db_prefix().' ware_color.color_name = "' . $color . '"';
			}
		}
	}
	if ($where_color_filter != '') {
		$where_color_filter .= ')';
		array_push($where, $where_color_filter);
	}
}
...
if (get_status_modules_wh('purchase')) {
	$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'items.id', db_prefix() . 'items.description', db_prefix() . 'items.unit_id', db_prefix() . 'items.commodity_code',db_prefix() . 'items.Width',db_prefix() . 'items.Length',db_prefix() . 'items.Thickness',db_prefix() . 'items.shape',  db_prefix() . 'items.commodity_barcode', db_prefix() . 'items.commodity_type', db_prefix() . 'items.warehouse_id', db_prefix() . 'items.origin', db_prefix() . 'items.color_id', db_prefix() . 'items.style_id', db_prefix() . 'items.model_id', db_prefix() . 'items.size_id',db_prefix() . 'items.material_type_id',
	db_prefix() . 'mrp_material.material_type_id',db_prefix() . 'items.profile_type_id',
	db_prefix() . 'mrp_profile.profile_type_id', db_prefix() . 'items.finish_type_id',db_prefix() . 'items.bend_type',db_prefix() . 'items.finish_side',
db_prefix() . 'items.no_of_groove',
db_prefix() . 'items.development_length',
db_prefix() . 'items.grooving_position',

	
	db_prefix() . 'mrp_finish.finish_type_id',  db_prefix() . 'items.rate', db_prefix() . 'items.tax', db_prefix() . 'items.group_id', db_prefix() . 'items.long_description', db_prefix() . 'items.sku_code', db_prefix() . 'items.sku_name',db_prefix() . 'items.finish_side', db_prefix() . 'items.sub_group', db_prefix() . 'items.color', db_prefix() . 'items.guarantee', db_prefix().'items.profif_ratio', db_prefix().'items.without_checking_warehouse', db_prefix().'items.parent_id', db_prefix().'items.tax2', db_prefix().'items.can_be_sold', db_prefix().'items.can_be_purchased', db_prefix().'items.can_be_manufacturing', db_prefix().'items.can_be_inventory', db_prefix().'ware_style_type.style_code', db_prefix().'ware_style_type.style_barcode', db_prefix().'ware_style_type.style_name', db_prefix().'ware_style_type.note' , db_prefix().'ware_body_type.body_code', db_prefix().'ware_body_type.body_name', db_prefix().'ware_body_type.note'  , db_prefix().'ware_size_type.size_code', db_prefix().'ware_size_type.size_name', db_prefix().'ware_size_type.size_symbol', db_prefix().'ware_size_type.note' , db_prefix().'ware_color.color_code', db_prefix().'ware_color.color_name', db_prefix().'ware_color.color_hex', db_prefix().'ware_color.note', db_prefix().'items.from_vendor_item',db_prefix().'items.Width']);
}else{

	$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'items.id', db_prefix() . 'items.description',db_prefix() . 'items.Width',db_prefix() . 'items.Length',db_prefix() . 'items.Thickness',db_prefix() . 'items.shape', db_prefix() . 'items.unit_id', db_prefix() . 'items.commodity_code', db_prefix() . 'items.commodity_barcode', db_prefix() . 'items.commodity_type', db_prefix() . 'items.warehouse_id', db_prefix() . 'items.origin', db_prefix() . 'items.color_id', db_prefix() . 'items.style_id', db_prefix() . 'items.model_id', db_prefix() . 'items.size_id',db_prefix() . 'items.material_type_id',
	db_prefix() . 'mrp_material.material_type_id', db_prefix() . 'items.profile_type_id',db_prefix() . 'items.bend_type',
db_prefix() . 'items.no_of_groove',
db_prefix() . 'items.development_length',
db_prefix() . 'items.grooving_position'
,
	db_prefix() . 'mrp_profile.profile_type_id', db_prefix() . 'items.finish_type_id',
	db_prefix() . 'mrp_finish.finish_type_id',  db_prefix() . 'items.rate', db_prefix() . 'items.tax', db_prefix() . 'items.group_id', db_prefix() . 'items.long_description', db_prefix() . 'items.sku_code', db_prefix() . 'items.sku_name', db_prefix() . 'items.sub_group', db_prefix() . 'items.color', db_prefix() . 'items.guarantee', db_prefix().'items.profif_ratio', db_prefix().'items.without_checking_warehouse', db_prefix().'items.parent_id', db_prefix().'items.tax2', db_prefix().'items.can_be_sold', db_prefix().'items.can_be_purchased', db_prefix().'items.can_be_manufacturing', db_prefix().'items.can_be_inventory', db_prefix().'ware_style_type.style_code', db_prefix().'ware_style_type.style_barcode', db_prefix().'ware_style_type.style_name', db_prefix().'ware_style_type.note' , db_prefix().'ware_body_type.body_code', db_prefix().'ware_body_type.body_name', db_prefix().'ware_body_type.note'  , db_prefix().'ware_size_type.size_code', db_prefix().'ware_size_type.size_name', db_prefix().'ware_size_type.size_symbol', db_prefix().'ware_size_type.note' , db_prefix().'ware_color.color_code', db_prefix().'ware_color.color_name', db_prefix().'ware_color.color_hex', db_prefix().'ware_color.note',db_prefix().'items.Width' ]);
}
...
'" data-grooving_position="' . $aRow['grooving_position'] . '" data-Width="' . $aRow['Width'] . '" data-Length="' . $aRow['Length'] . '" data-Thickness="' . $aRow['Thickness'] . '" data-shape="' . $aRow['shape'] . '" data-Width="' . $aRow['Width'] . '" data-Length="' . $aRow['Length'] . '" data-hs_code="' . $aRow['hs_code'].'" data-sku_code="' . $aRow['sku_code'] . '" data-sku_name="' . $aRow['sku_name'] . '" data-sub_group="' . $aRow['sub_group'] . '" data-purchase_price="' . $aRow['purchase_price'] . '" data-color="' . $aRow['color'] . '" data-guarantee="' . $aRow['guarantee'] . '" data-profif_ratio="' . $aRow['profif_ratio'] . '" data-without_checking_warehouse="' . $aRow['without_checking_warehouse'] . '" data-parent_id="' . $aRow['parent_id'] . '" data-tax2="' . $aRow['tax2'] . '" data-can_be_sold="' . $aRow['can_be_sold'] .  '" data-Width="' . $aRow['Width'] . '" data-can_be_purchased="' . $aRow['can_be_purchased'] . '" data-can_be_manufacturing="' . $aRow['can_be_manufacturing'] . '" data-can_be_inventory="' . $aRow['can_be_inventory'] . '"   >' . _l('edit') . '</a>';
				...
				$code = '<span class="label label-tag tag-id-1"><span class="tag">' . $aRow['sku_code'] . '</span><span class="hide">, </span></span>&nbsp';
			//	$code = '<a href="' . admin_url('warehouse/view_commodity_detail/' . $aRow['id']) . '">' . $aRow['commodity_code'] . '</a>';
				$code .= '<div class="row-options">';
				...
				if (has_permission('warehouse_item', '', 'edit') || is_admin()) {
					$code .= ' | <a href="#" onclick="edit_commodity_item(this); return false;"  data-commodity_id="' . $aRow['id'] . '" data-description="' . $aRow['description'] . '" data-unit_id="' . $aRow['unit_id'] . '" data-commodity_code="' . $aRow['commodity_code'] . '" data-commodity_barcode="' . $aRow['commodity_barcode'] . '" data-commodity_type="' . $aRow['commodity_type'] . '" data-origin="' . $aRow['origin'] . '" data-color_id="' . $aRow['color_id'] . '" data-style_id="' . $aRow['style_id'] . '" data-bend_type="' . $aRow['bend_type'] . 
'" data-finish_side="' . $aRow['finish_side'] . 
'" data-no_of_groove="' . $aRow['no_of_groove'] . 
'" data-development_length="' . $aRow['development_length'] . 
'" data-grooving_position="' . $aRow['grooving_position'] . '" data-model_id="' . $aRow['model_id'] . '" data-size_id="' . $aRow['size_id'] . '" data-finish_type_id="' . $aRow['finish_type_id'] . '" data-material_type_id="' . $aRow['material_type_id'] . '" data-profile_type_id="' . $aRow['profile_type_id'] . '" data-shape="' . $aRow['shape'] . '" data-Width="' . $aRow['Width'] . '" data-Length="' . $aRow['Length'] . '" data-Thickness="' . $aRow['Thickness'] . '" data-hs_code="' . $aRow['hs_code'].'"  data-rate="' . $aRow['rate'] . '" data-group_id="' . $aRow['group_id'] . '" data-tax="' . $aRow['tax'] . '"  data-warehouse_id="' . $aRow['warehouse_id'] . '" data-sku_code="' . $aRow['sku_code'] . '" data-sku_name="' . $aRow['sku_name'] . '" data-sub_group="' . $aRow['sub_group'] . '" data-purchase_price="' . $aRow['purchase_price'] . '" data-color="' . $aRow['color'] . '" data-guarantee="' . $aRow['guarantee'] . '" data-profif_ratio="' . $aRow['profif_ratio'] . '" data-without_checking_warehouse="' . $aRow['without_checking_warehouse'] . '" data-parent_id="' . $aRow['parent_id'] . '" data-tax2="' . $aRow['tax2'] . '" data-can_be_sold="' . $aRow['can_be_sold'] . '" data-can_be_purchased="' . $aRow['can_be_purchased'] . '" data-can_be_manufacturing="' . $aRow['can_be_manufacturing'] . '" data-can_be_inventory="' . $aRow['can_be_inventory'] . '"  >' . _l('edit') . '</a>';
				}
...
//  elseif ($aColumns[$i] == 'full_description') {
// 				$_data = $aRow['full_description'];
...
// 			}
			...
			elseif ($aColumns[$i] == 'profile_name') {
				...
				elseif ($aColumns[$i] == 'material_name') {
					...
					elseif ($aColumns[$i] == 'color_name') {
					$_data = $aRow['color_name'];
				}

							elseif ($aColumns[$i] == 'finish_name') {
					$_data = $aRow['finish_name'];
				}
				elseif ($aColumns[$i] == 'finish_side') {
					$_data = $aRow['finish_side'];
				} 

								elseif ($aColumns[$i] == 'Thickness') {
								$_data =  $aRow['Thickness'];
							} 
							elseif ($aColumns[$i] == 'bend_type') {
					$_data = $aRow['bend_type'];
				} 
				elseif ($aColumns[$i] == 'no_of_groove') {
					$_data = $aRow['no_of_groove'];
				} 

				elseif ($aColumns[$i] == 'grooving_position') {
					$_data = $aRow['grooving_position'];
				}
				elseif ($aColumns[$i] == 'development_length') {
					$_data = $aRow['development_length'];
				} 
 ...
 elseif ($aColumns[$i] == 'unit_id') {
			...
			elseif ($aColumns[$i] == 'shape') {
				$_data =  $aRow['shape'];
				 } 
elseif ($aColumns[$i] == 'Width') {
				$aRow['Width'] = $aRow['Width'] ?? 0;

				$_data =  $aRow['Width'];
			} 
				
				elseif ($aColumns[$i] == 'Length') {
				$_data =  $aRow['Length'];
			} 
