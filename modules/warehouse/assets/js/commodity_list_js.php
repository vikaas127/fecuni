<script>
  ...
  (function($) {
    ...
    var ProposalServerParams = {
      ...
      "color_name_filter": "[name='color_name_filter[]']",

      "sub_group_filter": "[name='sub_group_filter[]']",
      "thickness_filter": "[name='thickness_filter[]']",
      "length_filter": "[name='length_filter[]']",
  ...
  function expenseSubmitHandler(form){
    ...
    data.material_type_id = $('select[name="material_type_id"]').val();
   data.finish_type_id= $('select[name="finish_type_id"]').val();
      data.sheet_type_id= $('select[name="sheet_type_id"]').val();

    data.profile_type_id= $('select[name="profile_type_id"]').val();
    ...
    data.shape=$('input[name="shape"]').val();
    data.hs_code=$('input[name="hs_code"]').val();
    data.bend_type = $('select[name="bend_type"]').val();
    data.cuttype = $('select[name="cuttype"]').val();

    data.finish_side = $('select[name="finish_side"]').val();
    data.calculated_weight=$('input[name="calculated_weight"]').val();
    data.actual_weight = $('select[name="actual_weight"]').val();
    data.no_of_groove=$('input[name="no_of_groove"]').val();
    data.development_length=$('input[name="development_length"]').val();
    data.grooving_position=$('input[name="grooving_position"]').val();
...
(function () {
  const $modal = $('#commodity_list-add-edit'); // <-- your modal

  // cache wrappers produced by render_select/render_input
  const $wrap = (name) => $modal.find('[app-field-wrapper="'+name+'"]');

  const $sheetWrap       = $wrap('sheet_type_id');
  const $profileWrap     = $wrap('profile_type_id');

  // extra fields to show only for Finished goods
  const $bendWrap        = $wrap('bend_type');
  const $grooveWrap      = $wrap('no_of_groove');
  const $groovingPosWrap = $wrap('grooving_position');
  const $devLenWrap      = $wrap('development_length');
  const $cuttypeWrap     = $wrap('cuttype');
  const $finishSideWrap  = $wrap('finish_side');
  ...
  function hideAll() {
    ...
    $bendWrap.hide();
    $grooveWrap.hide();
    $groovingPosWrap.hide();
    $devLenWrap.hide();
    $cuttypeWrap.hide();
    $finishSideWrap.hide();
  }

  window.toggleSheetProfileFields = function toggleSheetProfileFields() {
    const groupLabel    = $modal.find('select[name="group_id"] option:selected').text().trim().toLowerCase();
    const subGroupLabel = $modal.find('select[name="sub_group"] option:selected').text().trim().toLowerCase();

    hideAll();

    // Raw material + Sheet  -> show Sheet Type
    if (groupLabel.includes('raw') && subGroupLabel.includes('sheet')) {
      $sheetWrap.show();
    }

    // Finished goods -> show Profile Type + the extra fields
    if (groupLabel.includes('finished')) {
      $profileWrap.show();
      $bendWrap.show();
      $grooveWrap.show();
      $groovingPosWrap.show();
      $devLenWrap.show();
      $cuttypeWrap.show();
      $finishSideWrap.show();
    }
  ...
  // Run when THIS modal opens (add + edit)
  $modal.on('shown.bs.modal', function () {
    hideAll();
    window.toggleSheetProfileFields();
  });

  // Run whenever group/subgroup changes inside THIS modal
  $modal.on('change', 'select[name="group_id"], select[name="sub_group"]', function () {
    window.toggleSheetProfileFields();
  });
})();
  ...
  function edit_commodity_item(invoker){
      "use strict";
      console.log('Editing Commodity Item with the following data:');
       console.log($(invoker).data());
       console.log('Material Type ID:', $(invoker).data('material_type_id'));
      ...
      $('#commodity_list-add-edit input[name="sku_code"]').val($(invoker).data('calculated_weight'));
      $('#commodity_list-add-edit input[name="sku_name"]').val($(invoker).data('actual_weight'))
      $('#commodity_list-add-edit input[name="purchase_price"]').val($(invoker).data('purchase_price'));
      $('#commodity_list-add-edit input[name="Width"]').val($(invoker).data('width'));
      $('#commodity_list-add-edit input[name="Length"]').val($(invoker).data('length'));
      $('#commodity_list-add-edit input[name="Thickness"]').val($(invoker).data('thickness'));
      $('#commodity_list-add-edit input[name="shape"]').val($(invoker).data('shape'));
      $('#commodity_list-add-edit input[name="hs_code"]').val($(invoker).data('hs_code'));
      if($(invoker).data('no_of_groove') != 0 ){
       $('#commodity_list-add-edit select[name="no_of_groove"]').val($(invoker).data('no_of_groove')).change();
      }else{

       $('#commodity_list-add-edit select[name="no_of_groove"]').val('').change();
     }
      $('#commodity_list-add-edit input[name="no_of_groove"]').val($(invoker).data('no_of_groove'));
      $('#commodity_list-add-edit input[name="development_length"]').val($(invoker).data('development_length'));
      $('#commodity_list-add-edit input[name="grooving_position"]').val($(invoker).data('grooving_position'));
    ...
    if ($(invoker).data('material_type_id') != 0) {
      $('#commodity_list-add-edit select[name="material_type_id"]').val($(invoker).data('material_type_id')).change();
    } else {
      $('#commodity_list-add-edit select[name="material_type_id"]').val('').change();
    }

    if ($(invoker).data('finish_type_id') != 0) {
      $('#commodity_list-add-edit select[name="finish_type_id"]').val($(invoker).data('finish_type_id')).change();
    } else {
      $('#commodity_list-add-edit select[name="finish_type_id"]').val('').change();
    }
    if ($(invoker).data('sheet_type_id') != 0) {
      $('#commodity_list-add-edit select[name="sheet_type_id"]').val($(invoker).data('sheet_type_id')).change();
    } else {
      $('#commodity_list-add-edit select[name="sheet_type_id"]').val('').change();
    }

    if ($(invoker).data('profile_type_id') != 0) {
      $('#commodity_list-add-edit select[name="profile_type_id"]').val($(invoker).data('profile_type_id')).change();
    } else {
      $('#commodity_list-add-edit select[name="profile_type_id"]').val('').change();
    }
    ...
    updateCommodityDescription();


  }

function updateCommodityDescription() {
  let material_type_id = $('select[name="material_type_id"] option:selected').text().trim();
  ...
  profile_type_id = cleanValue(profile_type_id);
  color = cleanValue(color);
  finish_type_id = cleanValue(finish_type_id);
  finish_side = cleanValue(finish_side);
  size_id = cleanValue(size_id);
...
$('select[name="material_type_id"], select[name="profile_type_id"], select[name="color"], select[name="finish_type_id"], select[name="finish_side"], select[name="size_id"], input[name="Thickness"], input[name="Length"]')
  ...
  function new_commodity_item(){
    ...
    $('#commodity_list-add-edit input[name="Width"]').val('');
    $('#commodity_list-add-edit input[name="Thickness"]').val('');
    $('#commodity_list-add-edit input[name="Length"]').val('');
    $('#commodity_list-add-edit input[name="shape"]').val('');
    $('#commodity_list-add-edit input[name="hs_code"]').val('');
    $('#commodity_list-add-edit select[name="bend_type"]').val('').change();
    $('#commodity_list-add-edit select[name="cuttype"]').val('').change();

        $('#commodity_list-add-edit select[name="finish_side"]').val('').change();
    $('#commodity_list-add-edit input[name="calculated_weight"]').val('');
    $('#commodity_list-add-edit input[name="actual_weight"]').val('');
    $('#commodity_list-add-edit select[name="sheet_type_id"]').val('').change();

    $('#commodity_list-add-edit input[name="no_of_groove"]').val('');
    $('#commodity_list-add-edit input[name="development_length"]').val('');
    $('#commodity_list-add-edit input[name="grooving_position"]').val('');
  ...
  var parent_id = $('select[name="parent_id"]').val();
      ...
      if(!check_id){
         ...
         if(response.parent_value.commodity_type != 0){
          ...
          if (response.parent_value.material_type_id != 0) {
            $('#commodity_list-add-edit select[name="material_type_id"]').val(response.parent_value.material_type_id).change();
          } else {
            $('#commodity_list-add-edit select[name="material_type_id"]').val('').change();
          }

          if (response.parent_value.finish_type_id != 0) {
            $('#commodity_list-add-edit select[name="finish_type_id"]').val(response.parent_value.finish_type_id).change();
          } else {
            $('#commodity_list-add-edit select[name="finish_type_id"]').val('').change();
          }
          if (response.parent_value.sheet_type_id != 0) {
            $('#commodity_list-add-edit select[name="sheet_type_id"]').val(response.parent_value.sheet_type_id).change();
          } else {
            $('#commodity_list-add-edit select[name="sheet_type_id"]').val('').change();
          }

          if (response.parent_value.profile_type_id != 0) {
            $('#commodity_list-add-edit select[name="profile_type_id"]').val(response.parent_value.profile_type_id).change();
          } else {
            $('#commodity_list-add-edit select[name="profile_type_id"]').val('').change();
          }
...
//add opening stock
  ...
  init_ajax_search('items', '#thickness_filter.ajax-search', undefined, admin_url + 'warehouse/wh_thickness_search_all');
  init_ajax_search('items', '#length_filter.ajax-search', undefined, admin_url + 'warehouse/wh_length_search_all');
...
function open_size_type_from_commodity() {
  $('#sizeTypeModal').modal('show');
}
$(function () {
  $('#sizeTypeForm').on('submit', function (e) {
    e.preventDefault();
    ...
    $.post(form.attr('action'), formData)
      .done(function (response) {
        ...
        if (data.success) {
          ...
          $('#size_id').append(newOption).selectpicker('refresh');

          $('#sizeTypeModal').modal('hide');
          form[0].reset();
          alert_float('success', data.message || 'Size added successfully.');
        } else {
          alert_float('danger', data.message || 'Something went wrong.');
        }

        // Update CSRF token if provided
        if (data.csrf_hash) {
          $('input[name="' + csrfData.token_name + '"]').val(data.csrf_hash);
          csrfData.hash = data.csrf_hash;
        }
      })
      .fail(function () {
        alert_float('danger', 'Server error while saving size.');
      });
  });
});
