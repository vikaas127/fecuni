...
public function delete_warehouse_permission($id)
    ...
    foreach ($tables as $table) {
        ...
        if ($this->db->table_exists($tableName)) {
            if ($id) {
                ...
                $result[$table] = $query->row_array(); // Single row
            } else {
                // Fetch all records
                $query = $this->db->get($tableName);
                $result[$table] = $query->result_array(); // Multiple rows
            }
        } else {
	...
	public function add_size_type($data, $id = false) {
		...
		public function add_profile_type($data, $id = false) {
		$affectedRows = 0;

		if (isset($data['hot_profile_type'])) {
			$hot_profile_type = $data['hot_profile_type'];
			unset($data['hot_profile_type']);
		}

		
			$type_detail = json_decode($hot_profile_type);

			$es_detail = [];
			$row = [];
			$rq_val = [];
			$header = [];
	        $header[] = 'profile_type_id';
			$header[] = 'profile_code';
			$header[] = 'profile_name';
		...
		foreach ($es_detail as $key => $value) {
			...
			if($value['profile_type_id'] != ''){
				$row['update'][] = $value;
			}else{
				unset($value['profile_type_id']);
				$row['insert'][] = $value;
			}
		...
		if(count($row['insert']) != 0){
			$affected_rows = $this->db->insert_batch(db_prefix().'mrp_profile', $row['insert']);
			if($affected_rows > 0){
				$affectedRows++;
			}
		}
		if(count($row['update']) != 0){
			$affected_rows = $this->db->update_batch(db_prefix().'mrp_profile', $row['update'], 'profile_type_id');
			if($affected_rows > 0){
				$affectedRows++;
			}
		}
		...
		public function add_material_type($data, $id = false) {
			$affectedRows = 0;

			if (isset($data['hot_material_type'])) {
				$hot_material_type = $data['hot_material_type'];
				unset($data['hot_material_type']);
			}

		
			$type_detail = json_decode($hot_material_type);
		...
		foreach ($es_detail as $key => $value) {
			...
			if($value['material_type_id'] != ''){
				$row['update'][] = $value;
			}else{
				unset($value['material_type_id']);
				$row['insert'][] = $value;
			}
		...
		if(count($row['insert']) != 0){
			$affected_rows = $this->db->insert_batch(db_prefix().'mrp_material', $row['insert']);
			if($affected_rows > 0){
				$affectedRows++;
			}
		}
		if(count($row['update']) != 0){
			$affected_rows = $this->db->update_batch(db_prefix().'mrp_material', $row['update'], 'material_type_id');
			if($affected_rows > 0){
				$affectedRows++;
			}
		}
	...
	public function add_finish_type($data, $id = false) {
			$affectedRows = 0;

			if (isset($data['hot_finish_type'])) {
				$hot_finish_type = $data['hot_finish_type'];
				unset($data['hot_finish_type']);
			}

		
			$type_detail = json_decode($hot_finish_type);

			$es_detail = [];
			$row = [];
			$rq_val = [];
			$header = [];
            $header[] = 'finish_type_id';
			...
			if($value['finish_type_id'] != ''){
				$row['update'][] = $value;
			}else{
				unset($value['finish_type_id']);
				$row['insert'][] = $value;
			}
			...
			if(count($row['insert']) != 0){
				$affected_rows = $this->db->insert_batch(db_prefix().'mrp_finish', $row['insert']);
				if($affected_rows > 0){
					$affectedRows++;
				}
			}
			if(count($row['update']) != 0){
				$affected_rows = $this->db->update_batch(db_prefix().'mrp_finish', $row['update'], 'finish_type_id');
				if($affected_rows > 0){
					$affectedRows++;
				}
			}
	...
	public function add_sheet_type($data, $id = false) {
			$affectedRows = 0;

			if (isset($data['hot_sheet_type'])) {
				$hot_sheet_type = $data['hot_sheet_type'];
				unset($data['hot_sheet_type']);
			}

		
			$type_detail = json_decode($hot_sheet_type);

			$es_detail = [];
			$row = [];
			$rq_val = [];
			$header = [];
            $header[] = 'sheet_type_id';
			...
			if($value['sheet_type_id'] != ''){
				$row['update'][] = $value;
			}else{
				unset($value['sheet_type_id']);
				$row['insert'][] = $value;
			}
			...
			if(count($row['insert']) != 0){
				$affected_rows = $this->db->insert_batch(db_prefix().'wh_sheet', $row['insert']);
				if($affected_rows > 0){
					$affectedRows++;
				}
			}
			if(count($row['update']) != 0){
				$affected_rows = $this->db->update_batch(db_prefix().'wh_sheet', $row['update'], 'sheet_type_id');
				if($affected_rows > 0){
					$affectedRows++;
				}
			}
	...
	public function get_profile_type($id = false) {

		if (is_numeric($id)) {
			$this->db->where('profile_type_id', $id);

			return $this->db->get(db_prefix() . 'mrp_profile')->row();
		}
		if ($id == false) {
			return $this->db->query('select * from tblmrp_profile')->result_array();
		}

	}
	public function get_material_type($id = false) {

		if (is_numeric($id)) {
			$this->db->where('material_type_id', $id);

			return $this->db->get(db_prefix() . 'mrp_material')->row();
		}
		if ($id == false) {
			return $this->db->query('select * from tblmrp_material')->result_array();
		}

	}
	public function get_finish_type($id = false) {

		if (is_numeric($id)) {
			$this->db->where('finish_type_id', $id);

			return $this->db->get(db_prefix() . 'mrp_finish')->row();
		}
		if ($id == false) {
			return $this->db->query('select * from tblmrp_finish')->result_array();
		}

	}
	public function get_sheet_type($id = false) {

		if (is_numeric($id)) {
			$this->db->where('sheet_type_id', $id);

			return $this->db->get(db_prefix() . 'wh_sheet')->row();
		}
		if ($id == false) {
			return $this->db->query('select * from tblwh_sheet')->result_array();
		}
	...
	public function get_material_add_commodity() {

		return $this->db->query('select * from tblmrp_material  order by tblmrp_material.material_type_id asc')->result_array();

	}	
	public function get_profile_add_commodity() {

		return $this->db->query('select * from tblmrp_profile  order by tblmrp_profile.profile_type_id asc')->result_array();

	}		
	public function get_finish_add_commodity() {

		return $this->db->query('select * from tblmrp_finish  order by tblmrp_finish.finish_type_id asc')->result_array();

	}	
	public function get_sheet_add_commodity() {

		return $this->db->query('select * from tblwh_sheet  order by tblwh_sheet.sheet_type_id asc')->result_array();
	...
	public function delete_size_type($id) {
		...
		$this->db->where('finish_type_id', $id);
		$this->db->delete(db_prefix() . 'mrp_finish');
		if ($this->db->affected_rows() > 0) {
			return true;
		}
		return false;
		...
		$this->db->where('profile_type_id', $id);
		$this->db->delete(db_prefix() . 'mrp_profile');
		if ($this->db->affected_rows() > 0) {
			return true;
		}
		return false;
		...
		$this->db->where('material_type_id', $id);
		$this->db->delete(db_prefix() . 'mrp_material');
		if ($this->db->affected_rows() > 0) {
			return true;
		}
		return false;
	...
	public function add_inventory_manage($data, $status) {
		...
		if ($status == 1) {
			...
			if($affected_rows > 0){
				$this->db->select('cm.commodity_name,im.commodity_id, im.inventory_number, cm.inventory_number_min')
			         ->from(db_prefix() . 'inventory_manage im')
			         ->join(db_prefix() . 'inventory_commodity_min cm', 'im.commodity_id = cm.commodity_id')
			         ->where('im.commodity_id', $data['commodity_code'])
			         ->where('im.inventory_number < cm.inventory_number_min');

				$query = $this->db->get();
				$result = $query->row();

				if ($result) {
					$subject = "Low Inventory Alert - " . $result->commodity_name;
				    $message = "
				        <p><strong>Commodity:</strong> {$result->commodity_name}</p>
				        <p><strong>Current Inventory:</strong> {$result->inventory_number}</p>
				        <p><strong>Minimum Required:</strong> {$result->inventory_number_min}</p>
				        <p>Please restock as soon as possible.</p>
				    ";
				   $this->send_low_inventory_alert($subject,$message);
				} 
				return true;
			}
			return false;
...
private function apply_inventory_match_conditions($data)
{
	...
	public function send_low_inventory_alert($subject,$message){
		...
		$this->db->select('email')
		         ...
		         ->where('admin', 1)
		         ->order_by('staffid', 'ASC') // Get the first admin
		         ->limit(1);
		$admin_query = $this->db->get();
		$admin = $admin_query->row();
		$this->load->model('emails_model');
		//send mail
		$check = $this->emails_model->send_simple_email($admin->email,$subject,$message);
	}
	/**
...
public function stock_export_pdf($delivery) {
    error_log("Starting stock export PDF generation for delivery ID: " . (is_object($delivery) ? $delivery->id : json_encode($delivery)));
    
    // Generate the PDF
    $pdf = app_pdf('delivery', module_dir_path(WAREHOUSE_MODULE_NAME, 'libraries/pdf/Delivery_pdf.php'), $delivery);
    
    if ($pdf) {
        error_log("Stock export PDF generated successfully for delivery ID: " . (is_object($delivery) ? $delivery->id : json_encode($delivery)));
    } else {
        error_log("Failed to generate stock export PDF for delivery ID: " . (is_object($delivery) ? $delivery->id : json_encode($delivery)));
    }
...
public function get_stock_export_pdf_html($goods_delivery_id) {
    $this->load->model('currencies_model');
    $base_currency = $this->currencies_model->get_base_currency();
    log_message('info', 'Base currency fetched: ' . json_encode($base_currency));
    
    $goods_delivery = $this->get_goods_delivery($goods_delivery_id);
    log_message('info', 'Goods delivery data fetched for ID: ' . $goods_delivery_id);
    
    $goods_delivery_detail = $this->get_goods_delivery_detail($goods_delivery_id);
    log_message('info', 'Goods delivery detail data fetched for ID: ' . $goods_delivery_id);

    $company_name = get_option('invoice_company_name');
    $address = get_option('invoice_company_address');
    $tax_data = $this->get_html_tax_delivery($goods_delivery_id);
    log_message('info', 'Company name and address fetched: ' . $company_name . ', ' . $address);
    ...
    log_message('info', "Formatted date: Day - $day, Month - $month, Year - $year");

    $warehouse_lotnumber_bottom_infor_option = get_warehouse_option('goods_delivery_pdf_display_warehouse_lotnumber_bottom_infor');
    log_message('info', 'Warehouse lot number option: ' . $warehouse_lotnumber_bottom_infor_option);
    ...
    if ($goods_delivery) {
        if (is_numeric($goods_delivery->customer_code)) {
            ...
            if ($customer_value) {
                ...
                log_message('info', 'Customer data found: ' . json_encode($customer_value));
            } else {
                log_message('error', 'Customer data not found for code: ' . $goods_delivery->customer_code);
            }
        }
    }

    $html = '';
    log_message('info', 'Starting HTML generation.');

    $html .= '<table class="table"><tbody><tr><td rowspan="2" width="50%" class="text-left">' . pdf_logo_url() . '</td><td class="text_right_weight "><h3>' . mb_strtoupper(_l('delivery')) . '</h3></td></tr>';
    $html .= '<tr><td class="text_right">#' . $goods_delivery->goods_delivery_code . '</td></tr></tbody></table><br><br><br>';

    $organization_info = '<div class="bill_to_color">';
    $organization_info .= format_organization_info();
    $organization_info .= '</div>';
    log_message('info', 'Organization info formatted.');

    $bill_to = '';
    $ship_to = '';
    if (isset($customer_value)) {
        $bill_to .= '<b>' . _l('Bill to') . '</b><div class="bill_to_color">';
        $bill_to .= format_customer_info($customer_value, 'invoice', 'billing');
        $bill_to .= '</div>';

        $ship_to .= '<br /><b>' . _l('ship_to') . '</b><div class="bill_to_color">';
        $ship_to .= format_customer_info($customer_value, 'invoice', 'shipping');
        $ship_to .= '</div>';
        log_message('info', 'Billing and shipping info formatted for customer.');
    }
    ...
    if (is_numeric($goods_delivery->invoice_id) && $goods_delivery->invoice_id != 0) {
        ...
        log_message('info', 'Invoice information added for delivery.');
    }

    $html .= '<table class="table"><tbody><tr><td rowspan="2" width="50%" class="text-left">' . $organization_info . '</td><td rowspan="2" width="50%" class="text_right">' . $bill_to . '</td></tr></tbody></table><br><br>';
    $html .= '<table class="table"><tbody><tr><td rowspan="2" width="50%" class="text-left"></td><td rowspan="2" width="50%" class="text_right">' . $ship_to . '</td></tr></tbody></table><br>';
    $html .= '<table class="table"><tbody><tr><td rowspan="2" width="50%" class="text-left"></td><td rowspan="2" width="50%" class="text_right">' . $invoice_date . '</td></tr></tbody></table><br><br><br>';

    log_message('info', 'Main customer and invoice information added to HTML.');

    $new_goods_delivery_detail = [];
    $new_goods_delivery_serials = [];
    if (!get_option('wh_shortened_form_pdf')) {
        $html .= '<table class="table"><tbody><tr><th class="thead-dark"><b>#</b></th><th class="thead-dark">' . _l('commodity_name') . '</th>';
        if ($warehouse_lotnumber_bottom_infor_option == 1) {
            $html .= '<th class="thead-dark">' . _l('warehouse_name') . '</th>';
        }
        $html .= '<th class="thead-dark">' . _l('quantity') . '</th><th class="thead-dark">' . _l('delivered') . '</th>';
        ...
        $html .= '<th class="thead-dark">' . _l('subtotal') . '</th><th class="thead-dark">' . _l('subtotal_after_tax') . '</th><th class="thead-dark">' . _l('total_money') . '</th></tr>';

        $subtotal = 0;
        foreach ($goods_delivery_detail as $delivery_key => $delivery_value) {
            log_message('info', "Processing delivery item $delivery_key: " . json_encode($delivery_value));
            
            $item_order = $delivery_key + 1;
            $commodity_name = get_commodity_name($delivery_value['commodity_code'])->description ?? '';
            $quantities = $delivery_value['quantities'] ?? '';
            $unit_price = $delivery_value['unit_price'] ?? '';
            $total_money = $delivery_value['total_money'] ?? '';
            $total_after_discount = $delivery_value['total_after_discount'] ?? '';
            $subtotal += (float)$quantities * (float)$unit_price;
            $item_subtotal = (float)$quantities * (float)$unit_price;
            ...
            if (isset($delivery_value['warehouse_id']) && ($delivery_value['warehouse_id'] != '')) {
                ...
                foreach ($arr_warehouse as $warehouseid) {
                    if ($warehouseid != '' && $warehouseid != '0') {
                        ...
                        if ($team) {
                            $warehouse_name .= '<span class="label label-tag tag-id-1"><span class="tag">' . $team->warehouse_name . '</span></span>';
                        }
                    }
                }
            }
            ...
            $html .= '<tr>';
            $html .= '<td><b>' . $item_order . '</b></td><td>' . $commodity_name . '</td>';
            if ($warehouse_lotnumber_bottom_infor_option == 1) {
                $html .= '<td>' . $warehouse_name . '</td>';
            }
            $html .= '<td>' . $quantities . '</td><td>' . $quantities . '</td>';
            if (get_warehouse_option('goods_delivery_pdf_display_outstanding') == 1) {
                $html .= '<td>0.0</td>';
            }
            $html .= '<td>' . app_format_money((float)$unit_price, '') . '</td>';
            $html .= '<td>' . app_format_money((float)$item_subtotal, '') . '</td><td>' . app_format_money((float)$total_money, '') . '</td><td>' . app_format_money((float)$total_after_discount, '') . '</td>';
            $html .= '</tr>';
        }
    ...
    log_message('info', 'HTML table for goods delivery details added.');

    $after_discount = $goods_delivery->after_discount ?? $goods_delivery->total_money;
    $shipping_fee = $goods_delivery->shipping_fee ?? 0;
    $total_discount = ($goods_delivery->total_discount ?? 0) + ($goods_delivery->additional_discount ?? 0);

    // Additional summary table for Subtotal, Discount, Shipping Fee, and Total Payment
    $html .= '<table class="table"><tbody>';
    $html .= '<tr><td class="text_left"><b>' . _l('Subtotal') . '</b></td><td class="text_right">' . $base_currency->symbol . app_format_money((float)$subtotal, '') . '</td></tr>';
    $html .= '<tr><td class="text_left"><b>' . _l('Total discount') . '</b></td><td class="text_right">' . $base_currency->symbol . app_format_money((float)$total_discount, '') . '</td></tr>';
    $html .= '<tr><td class="text_left"><b>' . _l('Shipping fee') . '</b></td><td class="text_right">' . $base_currency->symbol . app_format_money((float)$shipping_fee, '') . '</td></tr>';
    $html .= '<tr><td class="text_left"><b>' . _l('Total payment') . '</b></td><td class="text_right">' . $base_currency->symbol . app_format_money((float)$after_discount - (float)$total_discount + (float)$shipping_fee, '') . '</td></tr>';
    $html .= '</tbody></table><br><br><br>';

    log_message('info', 'Subtotal, tax, discount, and shipping fee information added to HTML.');

    // Add date and signature placeholders
    $html .= '<div style="text-align: left; margin-top: 20px;">';
    $html .= '<p>Day .......... Month .......... Year ..........</p>';
    $html .= '</div>';
    $html .= '<table class="table" style="margin-top: 30px;"><tbody>';
    $html .= '<tr><td>Receiver</td><td>Stocker</td><td>Chief Accountant</td></tr>';
    $html .= '<tr><td>(Sign, full name)</td><td>(Sign, full name)</td><td>(Sign, full name)</td></tr>';
    $html .= '</tbody></table><br>';

    log_message('info', 'Signature and date placeholders added to HTML.');

    $html .= '<link href="' . FCPATH . 'modules/warehouse/assets/css/pdf_style.css" rel="stylesheet" type="text/css" />';
    log_message('info', 'CSS link added to HTML.');

    log_message('info', 'HTML generation complete. Returning HTML.');
    return $html;
}
	...
	public function add_commodity_one_item($data) {
	      log_message('debug', 'Item Model Data: ' . print_r($data, true));
		$arr_insert_cf=[];
		$arr_variation=[];
		$arr_attributes=[];
		/*get custom fields*/
		...
		$insert_id = $this->db->insert_id();
		  ...
		  // Log success or failure after insertion
		  if ($this->db->affected_rows() > 0) {
				log_message('debug', 'Item added successfully, Item ID: ' . $insert_id);
			} else {
				log_message('error', 'Error adding item: ' . $this->db->last_query());
			}
	...
	public function import_xlsx_commodity($data, $flag_insert_id) {
	    	log_message('debug', 'Row Model'.$row.' | Data: '.json_encode($data));
		//update column unit name use sales/items
...
if($data['commodity_code']!=null){
   ...
   public function update_po_detail_quantity($po_id, $goods_receipt_detail)
	...
	// 		if (isset($data['rack_shelf_data'])) {
	// 			$rack_shelf_data = json_decode($data['rack_shelf_data'], true);

	// 			if (!empty($rack_shelf_data)) {
...
public function add_one_warehouse($data) 
{
	...
	// 	// Handle hide_warehouse_when_out_of_stock checkbox
	...
	// 	unset($data['lots_data']);

	// 	$affectedRows = 0;

	// 	// Update warehouse base data
	// 	$this->db->where('warehouse_id', $id);
	// 	$this->db->update(db_prefix() . 'warehouse', $data);
	// 	if ($this->db->affected_rows() > 0) {
	// 		$affectedRows++;
	...
	// 	// Update staff assignment (only if admin)
	// 	if (is_admin()) {
	// 		$this->assignWarehouseToStaffs($id, $assign_to_staffs);
	// 	}

	// 	// Delete existing racks & shelves
	// 	$this->db->where('warehouse_id', $id);
	...
	// 	// Reinsert updated racks & shelves
	// 	foreach ($rack_shelf_data as $rack) {
	// 		$this->db->insert(db_prefix() . 'wr_rack', [
	// 			'warehouse_id' => $id,
	// 			'rack_name'    => $rack['rack_name']
...
public function wh_thickness_search($q)
{
    $this->db->select('DISTINCT(Thickness) as id, Thickness as name');
    $this->db->from(db_prefix() . 'items');
    $this->db->like('Thickness', $q);
    $this->db->where('Thickness !=', '');
    $this->db->order_by('Thickness', 'asc');
    $this->db->limit(100);

    return $this->db->get()->result_array();
}
public function wh_length_search($q)
{
    $this->db->select('DISTINCT(Length) as id, Length as name');
    $this->db->like('Length', $q);
    $this->db->where('Length !=', '');
    $this->db->where('Length IS NOT NULL');
...
{
    log_message('info', 'delete_Manufacturing_permission called with ID: ' . $id);
    ...
    log_message('debug', 'SQL WHERE clause: ' . $sql_where);
    ...
    if ($this->db->affected_rows() > 0) {
        log_message('info', 'Successfully deleted permissions for staff ID: ' . $id);
        return true;
    }

    log_message('warning', 'No permissions deleted for staff ID: ' . $id);
    return false;
}
