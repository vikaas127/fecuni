...
public function delete_reimbursement($id)
{
    ...
    if ($this->input->post()) {
        log_message('debug', 'Controller triggered: setting_earnings_list_hr_records');

        $data = $this->input->post();

        // Call the model
        $result = $this->hr_payroll_model->earnings_list_synchronization($data);

        if ($result) {
            set_alert('success', _l('hrp_updated_successfully'));
        } else {
            set_alert('warning', _l('hrp_nothing_updated'));
        }
	...
	public function manage_employees() {
		...
		log_message('info', 'HRP Columns Received: ' . print_r($format_employees_value['column_format'], true));
		...
		foreach ($staffs as $staff_key => $staff_value) {
			...
			if (isset($employees_value[$staff_value['staffid'] . '_' . $current_month])) {
				...
				// array merge: staff information + earning list (probationary contract) + earning list (formal)
				$data_object_kpi[$staff_key] = array_merge($data_object_kpi[$staff_key], $format_employees_value['formal']);
