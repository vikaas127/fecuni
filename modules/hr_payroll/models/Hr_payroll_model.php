...
{
    $affectedRows = 0;
    $hr_payroll_alphabeticala = hr_payroll_alphabeticala(); // ['A', 'B', 'C', ...]
    $array_salary_allowance = [];

    log_message('debug', '=== earnings_list_synchronization() called ===');
    ...
    if (hr_payroll_get_status_modules('hr_profile') && get_hr_payroll_option('integrated_hrprofile') == 1) {
        ...
        log_message('debug', 'Salary types: ' . count($salary_types));
        log_message('debug', 'Allowance types: ' . count($allowance_types));

        $used_codes = [];

        // Helper to get the next unique alphabetical code
        $get_next_code = function () use (&$hr_payroll_alphabeticala, &$used_codes) {
            while (!empty($hr_payroll_alphabeticala)) {
                $code = array_shift($hr_payroll_alphabeticala);
        ...
        foreach ($allowance_types as $value) {
            $code = $get_next_code();
            $array_salary_allowance[] = [
                'code'        => $code,
                'description' => $value['type_name'],
    ...
    // Step 1: Truncate all old entries
    $this->db->truncate(db_prefix() . 'hrp_earnings_list_hr_records');
    $affectedRows++;

    // Step 2: Insert fresh records
    if (!empty($array_salary_allowance)) {
        $inserted = $this->db->insert_batch(db_prefix() . 'hrp_earnings_list_hr_records', $array_salary_allowance);
        if ($inserted > 0) {
            log_message('debug', 'Inserted ' . count($array_salary_allowance) . ' earnings records.');
            $affectedRows++;
        } else {
            log_message('error', 'Insert failed: No records inserted.');
        }
    } else {
        log_message('debug', 'No earnings data prepared for insertion.');
    }
...
";
					...
					if($rel_type == 'hr_records'){
						...
						if(preg_match('/^st1_/', $key) || preg_match('/^al1_/', $key)){
							...
							// if(($key == 'probationary_effective' || $key == 'probationary_expiration' || $key == 'primary_effective' || $key == 'primary_expiration')  )
								if(( $key == 'primary_effective' || $key == 'primary_expiration')  ){
								if($value != ''){
									$temp[$key] = $value;
								}else{
									$temp[$key] = null;
	...
	{	
	    ...
	    $results['header']		= array_merge($staff_information_header,   $array_earnings_list_formal_header, $primary_date);
	...
	public function employees_synchronization($data)
{
	...
	log_message('info', '🔄 [START] employees_synchronization triggered with POST data: ' . print_r($data, true));
	...
	log_message('info', '📑 Staff Info Keys: ' . print_r($staff_information_key, true));
	// log_message('info', '📑 Probationary Earnings Keys: ' . print_r($staff_probationary_key, true)); // 🔒 Probationary Removed
	log_message('info', '📑 Formal Earnings Keys: ' . print_r($staff_formal_key, true));
	// log_message('info', '📅 Probationary Dates Keys: ' . print_r($probationary_key, true)); // 🔒 Probationary Removed
	log_message('info', '📅 Primary Dates Keys: ' . print_r($primary_key, true));
	...
	log_message('info', '📦 Contracts fetched: ' . print_r($staff_contract, true));
	...
	if(isset($hrp_employees_value)){
		...
		$header = array_merge($staff_information_key, $staff_formal_key, $primary_key);
		...
		foreach ($hrp_employees_detail as $key => $value) {				
			...
			foreach ($combine_data as $combine_key => $combine_value) {
				if($rel_type == 'hr_records'){
					...
					// 	}
					// 	array_push($probationary_contracts, [ $combine_key => $combine_value ]);
					// } 
					
					if(preg_match('/^st2_/', $combine_key) || preg_match('/^al2_/', $combine_key)){
						$combine_value = 0;
						...
						array_push($primary_contracts, [ $combine_key => $combine_value ]);
					}
			...
			log_message('info', "✅ Final Row Data for staff_id {$combine_data['staff_id']}: " . print_r($temp, true));
	...
	log_message('info', '🆕 Insert Rows: ' . print_r($row['insert'], true));
	log_message('info', '✏️ Update Rows: ' . print_r($row['update'], true));
	log_message('info', '🗑️ Delete IDs: ' . print_r($row['delete'], true));
	...
	if($data['department_employees_filter'] == '' && $data['staff_employees_filter'] == '' && $data['role_employees_filter'] == ''){
		...
		if($this->db->affected_rows() > 0){
			$affectedRows++;
			log_message('info', '🗑️ Records deleted from DB based on diff.');
		}
	}
	...
	if(count($row['insert']) != 0){
		...
		if($affected_rows > 0){
			$affectedRows++;
			log_message('info', "✅ Inserted {$affected_rows} rows into DB.");
		}
	}
	if(count($row['update']) != 0){
		...
		if($affected_rows > 0){
			$affectedRows++;
			log_message('info', "♻️ Updated {$affected_rows} rows in DB.");
		}
	}
	...
	log_message('info', '✅ [END] Synchronization process finished. Total affected: ' . $affectedRows);
...
public function reimbursements_update($data)
{
	...
	public function get_list_payroll_column_function_name($data)
	{
		...
		foreach ($salary_reimbursements as $key => $value) {
			...
			$list_column_method[] = ['function_name' => 'reimbursement_'.$value['id'],  'lable' => $name.' '];
		}
