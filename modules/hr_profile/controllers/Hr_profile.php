...
class Hr_profile extends AdminController {
	 ...
	 public function calculate_tax()
    {
        ...
        $ctc = $this->hr_profile_model->get_ctc($employee_id);
        $declaration = $this->hr_profile_model->get_latest_declaration($employee_id);

        if (!$declaration) {
            echo json_encode(['tax' => 0, 'error' => 'No declaration found']);
            return;
        }

        $deductions = $this->hr_profile_model->get_deductions($declaration, $regime, $employee_id);
        $taxable_income = max(0, $ctc - 50000 - $deductions);

        $tax = $this->hr_profile_model->calculate_tax_by_regime($taxable_income, $regime);

        echo json_encode(['tax' => round($tax)]);
    }
...
public function view_tax()
{
    ...
    log_message('info', 'view_tax called: employee_id=' . $employee_id . ', year=' . $financial_year);

    if (!$employee_id || !$financial_year) {
        log_message('error', 'view_tax missing required params');
        echo json_encode(['error' => 'Missing required parameters.']);
        return;
    }

    $result = $this->hr_profile_model->calculate_tax_both_regimes($employee_id, $financial_year);

    log_message('info', 'Tax result: ' . print_r($result, true));

    header('Content-Type: application/json'); // Ensure correct type
    echo json_encode($result);
}


public function save_tax_regime()
{
    $employee_id = $this->input->post('employee_id');
    $declaration_id = $this->input->post('declaration_id');
    $tax_regime = $this->input->post('tax_regime');

    if (!$employee_id || !$declaration_id || !$tax_regime) {
        echo json_encode(['error' => 'Missing parameters']);
        return;
    }

    $this->db->where('id', $declaration_id);
    $this->db->where('employee_id', $employee_id);
    $updated = $this->db->update(db_prefix() . 'hr_investment_declarations', [
...
// Controller: HR_Profile.php
// public function save_tax_regime()
// {
//     $declaration_id = $this->input->post('declaration_id');
//     $tax_regime = $this->input->post('tax_regime');

//     if (!$declaration_id || !$tax_regime) {
...
//     } else {
//         echo json_encode(['status' => false, 'message' => 'Failed to save tax regime']);
//     }
// }
 ...
 */
    ...
    return $serial; // Return as-is if already a string
}
	/**
				...
				if (!empty($tmpFilePath) && $tmpFilePath != '') {
					...
					if (move_uploaded_file($tmpFilePath, $newFilePath)) {
						...
						$writer_header = array(
							...
							_l('hr_middle_name') => 'string',

							_l('hr_lastname') => 'string',
							_l('hr_father') => 'string',
							_l('hr_husband_wife')=> 'string',
                            _l('hr_disability_type')=> 'string',
							_l('pan_no')=> 'string',
							_l('adhar_no')=> 'string',
							_l('hr_ifsc_code')=> 'string',
							_l('hr_sex') => 'string',
						...
						$widths = [40, 40, 40, 50, 40, 40, 40, 40, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50,50,50,50,50,50,50];
						...
						$col_style2 = [2, 4, 5,8,9,10,11,13,16,17];
						...
						$arr_header['middle_name'] = 3;
						$arr_header['lastname'] = 4;
						$arr_header['father_name'] = 5;
						$arr_header['ho_wo_name'] = 6;
						$arr_header['disability_type'] = 7;
						$arr_header['pan_no'] = 8;
						$arr_header['adhar_no'] = 9;
						$arr_header['ifsc_code'] = 10;
						$arr_header['sex'] = 11;
						$arr_header['birthday'] = 12;
						$arr_header['email'] = 13;
						$arr_header['phonenumber'] = 14;
						$arr_header['workplace'] = 15;
						$arr_header['status_work'] = 16;
						$arr_header['job_position'] = 17;
						$arr_header['team_manage'] = 18;
						$arr_header['role'] = 19;
						$arr_header['literacy'] = 20;
						$arr_header['hourly_rate'] = 21;
						$arr_header['department'] = 22;
						$arr_header['password'] = 23;
						$arr_header['home_town'] = 24;
						$arr_header['marital_status'] = 25;
						$arr_header['current_address'] = 26;
						$arr_header['nation'] = 27;
						$arr_header['birthplace'] = 28;
						$arr_header['religion'] = 29;
						$arr_header['identification'] = 30;
						$arr_header['days_for_identity'] = 31;
						$arr_header['place_of_issue'] = 32;
						$arr_header['resident'] = 33;
						$arr_header['account_number'] = 34;
						$arr_header['name_account'] = 35;
						$arr_header['issue_bank'] = 36;
						$arr_header['Personal_tax_code'] = 37;
						$arr_header['facebook'] = 38;
						$arr_header['linkedin'] = 39;
						$arr_header['skype'] = 40;
						...
						for ($row = 2; $row < count($data); $row++) {
							...
							$value_middle_name = isset($data[$row][$arr_header['middle_name']]) ? $data[$row][$arr_header['middle_name']] : '';
							$value_lastname = isset($data[$row][$arr_header['lastname']]) ? $data[$row][$arr_header['lastname']] : '';
							$value_father_name = isset($data[$row][$arr_header['father_name']]) ? $data[$row][$arr_header['father_name']] : '';
							$value_ho_wo = isset($data[$row][$arr_header['ho_wo_name']]) ? $data[$row][$arr_header['ho_wo_name']] : '';
							$value_disability_type = isset($data[$row][$arr_header['disability_type']]) ? $data[$row][$arr_header['disability_type']] : '';
							$value_pan_no = isset($data[$row][$arr_header['pan_no']]) ? $data[$row][$arr_header['pan_no']] : '';
							$value_adhar_no = isset($data[$row][$arr_header['adhar_no']]) ? $data[$row][$arr_header['adhar_no']] : '';
							$value_ifsc_code = isset($data[$row][$arr_header['ifsc_code']]) ? $data[$row][$arr_header['ifsc_code']] : '';
							...
							if (is_null($value_father_name) == true || $value_father_name == '') {
								$string_error .= _l('hr_father') . ' ' . _l('not_yet_entered') . '; ';
								$flag = 1;
							}
							if (is_null($value_pan_no) == true || $value_pan_no == '') {
								$string_error .= _l('pan_no') . ' ' . _l('not_yet_entered') . '; ';
								$flag = 1;
							}
							if (is_null($value_adhar_no) == true || $value_adhar_no == '') {
								$string_error .= _l('adhar_no') . ' ' . _l('not_yet_entered') . '; ';
								$flag = 1;
							}if (is_null($value_ifsc_code) == true || $value_ifsc_code == '') {
								$string_error .= _l('hr_ifsc_code') . ' ' . _l('not_yet_entered') . '; ';
								$flag = 1;
							}
							...
							//check start_time
                            $value_birthday = $this->excel_serial_to_date($value_birthday);
							...
							if (($flag == 1) || $flag2 == 1) {
								...
								log_message('error', "Row $row FAILED. Staff ID: $value_staffid, Email: $value_email, Error(s): $string_error | 
Values => 
First Name: '$value_firstname', 
Middle Name: '$value_middle_name', 
Last Name: '$value_lastname', 
Father: '$value_father_name', 
Husband/Wife: '$value_ho_wo', 
Disability Type: '$value_disability_type', 
PAN: '$value_pan_no', 
Aadhar: '$value_adhar_no', 
IFSC: '$value_ifsc_code', 
Sex: '$value_sex', 
Birthday: '$value_birthday', 
Phone: '$value_phonenumber', 
Workplace: '$value_workplace', 
Status Work: '$value_status_work', 
Job Position: '$value_job_position', 
Team Manage: '$value_team_manage', 
Role: '$value_role', 
Literacy: '$value_literacy', 
Hourly Rate: '$value_hourly_rate', 
Department: '$value_department', 
Password: '$value_password', 
Home Town: '$value_home_town', 
Marital Status: '$value_marital_status', 
Current Address: '$value_current_address', 
Nation: '$value_nation', 
Birthplace: '$value_birthplace', 
Religion: '$value_religion', 
Identification: '$value_identification', 
Days For Identity: '$value_days_for_identity', 
Place of Issue: '$value_place_of_issue', 
Resident: '$value_resident', 
Account Number: '$value_account_number', 
Name Account: '$value_name_account', 
Issue Bank: '$value_issue_bank', 
Personal Tax Code: '$value_Personal_tax_code', 
...
Skype: '$value_skype'"
							...
							if ($flag == 0 && $flag2 == 0) {
								...
								$rd['middle_name'] = $value_middle_name;
								$rd['lastname'] = $value_lastname;
								$rd['father_name'] = $value_father_name;
								$rd['ho_wo_name'] = $value_ho_wo;
								$rd['disability_type'] = $value_disability_type;
								$rd['pan_no'] = $value_pan_no;
								$rd['adhar_no'] = $value_adhar_no;
								$rd['ifsc_code'] = $value_ifsc_code;
								$rd['sex'] = $value_sex;
	...
	public function member($id = '', $group = '') {
		...
		if ($id == '') {
			...
			if ($data['group'] == 'profile') {
				$declaration = $this->hr_profile_model->get_latest_declaration_by_staff($id);

				$data['declaration'] = $declaration;
				$declaration_id = $declaration ? $declaration->id : null;
				
				if ($declaration_id) {
					$data['hra_data'] = $this->hr_profile_model->get_hra_details($declaration_id);
					$data['loan_data'] = $this->hr_profile_model->get_loan_details($declaration_id);
					$data['investment_data'] = $this->hr_profile_model->get_investment_items($declaration_id);
			...
			if ($data['group'] == 'investments') {
               $declaration = $this->hr_profile_model->get_latest_declaration_by_staff($id);

                $data['declaration'] = $declaration ? (array) $declaration : [];
				$declaration_id = $declaration ? $declaration->id : null;
                $data['financial_year'] = $declaration ? $declaration->financial_year : null;
				...
				$data['investment_sections'] = $this->get_investments_sections();
...
public function investments() {
    if ($this->input->post()) {
        ...
        // Log raw input
        log_message('debug', 'HR Investments POST data: ' . json_encode($data));

        // Sanitize staffid
        $data['staffid'] = isset($data['staffid']) && !empty($data['staffid'])
            ? (int)$data['staffid']
            : get_staff_user_id();

        log_message('info', 'HR Investments action initiated by staff ID: ' . $data['staffid']);

        $this->load->model('hr_profile_model');

        $success = $this->hr_profile_model->add_edit_investments($data, $data['staffid']);

        if ($success) {
            log_message('info', 'HR Investments saved successfully for staff ID: ' . $data['staffid']);
            set_alert('success', _l('Data saved successfully.'));
        } else {
            log_message('error', 'HR Investments failed to save for staff ID: ' . $data['staffid']);
            set_alert('danger', _l('Something went wrong.'));
        }

        // Determine redirect path
        $redirect_staff_id = $data['staffid'];
        $manage = isset($data['manage']) ? $data['manage'] : false;

        if ($manage) {
            redirect(admin_url('hr_profile/investments'));
        } else {
            redirect(admin_url('hr_profile/member/' . $redirect_staff_id . '/investments'));
        }

    } else {
        log_message('debug', 'HR Investments accessed without POST data.');
    }
}
    ...
    if (preg_match('/^st_/', $rel_type)) {
        $form_id = new_str_replace('st_', '', $rel_type);
        $salary_type = $this->hr_profile_model->get_salary_form($form_id);

        $type = 'salary';
        if ($salary_type) {
            $calculation_type = (int)$salary_type->calculation_type;
            $percentage_value = (float)$salary_type->salary_val;

            if ($calculation_type === 0) {
                $rel_value = $percentage_value;
                $annual_value = $rel_value * 12;
            } elseif ($calculation_type === 2 && $this->input->post('ctc')) {
                $ctc = (float)$this->input->post('ctc');
                $rel_value = round(($percentage_value * $ctc) / (12 * 100), 2);
                $annual_value = round(($percentage_value * $ctc) / 100, 2);
            } elseif ($calculation_type === 1 && $this->input->post('basic')) {
                $basic = (float)$this->input->post('basic');
                $rel_value = round(($percentage_value * $basic) / 100, 2);
                $annual_value = round($rel_value * 12, 2);
            }
        }

    } elseif (preg_match('/^al_/', $rel_type)) {
        $form_id = new_str_replace('al_', '', $rel_type);
        $allowance_type = $this->hr_profile_model->get_allowance_type($form_id);

        $type = 'allowance';
        if ($allowance_type) {
            $rel_value = (float)$allowance_type->allowance_val;
            $percentage_value = $rel_value;
            $calculation_type = 0;
            $annual_value = $rel_value * 12;
        }
    }
	...
	// 			$data['member'] = $this->staff_model->get($staff_id);
	// 		}
	// 		$declaration = $this->db->where('employee_id', $staff_id)->get(db_prefix() . 'hr_investment_declarations')->row_array();

	// 		if ($declaration) {
	// 			$data['declaration'] = $declaration;
	// 			$data['declaration_id'] = $declaration['id'];
	// 			$data['tax_regime'] = $declaration['tax_regime'];

	// 			$data['hra_data'] = $this->db->where('declaration_id', $declaration['id'])->get(db_prefix() . 'hr_hra_details')->result_array();
	// 			$data['loan_data'] = $this->db->where('declaration_id', $declaration['id'])->get(db_prefix() . 'hr_home_loan_details')->result_array();
	// 			$data['investment_data'] = $this->db->where('declaration_id', $declaration['id'])->get(db_prefix() . 'hr_investment_items')->result_array();
	// 		} else {
	...
	public function member_modal() {
		...
		if ($this->input->post('slug') === 'create') {
			$data = []; // ensure defined
			$this->load->view('hr_record/new_member', $data); // fix typo if needed
		} else if ($this->input->post('slug') === 'update') {
			$staff_id = $this->input->post('staff_id');
			$role_id = $this->input->post('role_id');
			$data = ['funcData' => ['staff_id' => $staff_id]];
			...
			$add_new = $this->input->post('add_new');
			$data['add_new'] = $add_new == ' hide' ? ' hide' : '';
			$data['display_staff'] = $add_new == ' hide' ? '' : ' hide';

			$this->load->model('currencies_model');
			$data['list_staff'] = $this->staff_model->get();
			$data['base_currency'] = $this->currencies_model->get_base_currency();
			$data['disability_types'] = $this->get_disability_types();
			$data['section'] = $this->get_investments_sections();
	...
	private function get_investments_sections()
{
    return [
        '80C'      => '80C',
        '80D'      => '80D',
...
'Interest on National Savings Certificates'];
 break;
        case '80CCC':
            $types = ['Contribution to annuity plan of LIC'];
			 break;
        case '80CCD':
            $types = ['National Pension Scheme'];
 break;
       
        case '80D':
            $types = ['Medi Claim Policy for self, spouse, children - 80D',
...
'Medical Bills for parents for senior citizen - 80D'
            ...
            break;
    case '80DD':
    $types = ['Treatment of dependent with disability', 'Treatment of dependent with severe disability'];
    break;

case '80DDB':
    $types = ['Medical expenditure for self or dependent', 'Medical expenditure for self or dependent for senior citizen', 'Medical expenditure for self or dependent for very senior citizen'];
    break;

case '80E':
    $types = ['Interest paid on Education loan'];
    break;

case '80EE':
    $types = ['Additional interest on housing loan borrowed between 1 Apr 2016 and 31 Mar 2017'];
    break;

case '80EEA':
    $types = ['Additional interest on housing loan borrowed between 1 Apr 2019 and 31 Mar 2022'];
    break;

case '80EEB':
    $types = ['Interest on electric vehicle loan borrowed between 1 Apr 2019 and 31 Mar 2023'];
    break;

case '80G':
    $types = ['Donation eligible for 100% exemption', 'Donation eligible for 50% exemption'];
    break;

case '80GGC':
    $types = ['Donation for political party'];
    break;

case '80TTA':
    $types = ['Interest from Savings Account'];
    break;

case '80U':
    $types = ['Permanent physical disability (self)', 'Permanent severe physical disability (self)'];
    break;
...
private function get_disability_types()
{
    return [
        'none'      => 'None',
...
//             // Log and insert
//             log_message('debug', 'Final data sent to add_staff: ' . print_r($data, true));
//             $id = $this->hr_profile_model->add_staff($data);
...
//             // ✅ Extract investment-tab fields
//             $hra = $data['hra'] ?? [];
//             $loans = $data['loans'] ?? [];
//             $investments = $data['investments'] ?? [];
//             $tax_regime = $data['tax_regime'] ?? null;

//             // ✅ Remove from $data so it's not saved to `staff` table
//             unset($data['hra'], $data['loans'], $data['investments'], $data['tax_regime']);

//             // ✅ Update main staff record
//             $response = $this->hr_profile_model->update_staff($data, $id);

//             // ✅ Update investment tables
//             $declaration = $this->db->where('employee_id', $id)->get(db_prefix() . 'hr_investment_declarations')->row_array();
//             if ($declaration) {
//                 $declaration_id = $declaration['id'];

//                 if ($tax_regime) {
//                     $this->db->where('id', $declaration_id)->update(db_prefix() . 'hr_investment_declarations', [
//                         'tax_regime' => $tax_regime,
//                     ]);
//                 }

//                 $this->db->where('declaration_id', $declaration_id)->delete(db_prefix() . 'hr_hra_details');
//                 foreach ($hra as $row) {
//                     $this->db->insert(db_prefix() . 'hr_hra_details', [
//                         'declaration_id' => $declaration_id,
//                         'rental_from'    => $row['rental_from'] ?? '',
//                         'rental_to'      => $row['rental_to'] ?? '',
...
//                 $this->db->where('declaration_id', $declaration_id)->delete(db_prefix() . 'hr_home_loan_details');
//                 foreach ($loans as $row) {
//                     $this->db->insert(db_prefix() . 'hr_home_loan_details', [
//                         'declaration_id'   => $declaration_id,
//                         'lender_name'      => $row['lender_name'] ?? '',
//                         'loan_start_date'  => $row['loan_start_date'] ?? '',
//                         'annual_principal' => $row['annual_principal'] ?? 0,
//                         'annual_interest'  => $row['annual_interest'] ?? 0,
//                         'property_address' => $row['property_address'] ?? '',
//                     ]);
//                 }

//                 $this->db->where('declaration_id', $declaration_id)->delete(db_prefix() . 'hr_investment_items');
//                 $total = 0;
//                 foreach ($investments as $row) {
//                     $amount = is_numeric($row['declared_amount']) ? floatval($row['declared_amount']) : 0;
//                     $this->db->insert(db_prefix() . 'hr_investment_items', [
//                         'declaration_id'  => $declaration_id,
//                         'section'         => $row['section'] ?? '',
...
//     if ($id != '') {
//         $data['member'] = $this->hr_profile_model->get_staff($id);
//         $data['employee'] = (array) $data['member'];

//         //  Load previous investment data
//         $declaration = $this->db->where('employee_id', $id)->get(db_prefix() . 'hr_investment_declarations')->row_array();
//         if ($declaration) {
//             $data['declaration_id'] = $declaration['id'];
//             $data['tax_regime'] = $declaration['tax_regime'];
...
// }
	...
	public function add_edit_member($id = '')
	{
		...
		if ($this->input->post()) {
			$data = $this->input->post();
			log_message('debug', 'Full $_FILES content: ' . print_r($_FILES, true));

		
			log_message('debug', 'Raw POST data: ' . print_r($data, true));
			...
			if ($id == '') {
				...
				redirect(admin_url($manage_staff ? 'hr_profile/staff_infor' : 'hr_profile/member/' . $id));
			}
		}
		...
		$data['list_staff']      = $this->staff_model->get();
		if ($id != '') {
			$existing = $this->hr_profile_model->get_staff($id);
			$data['staff_code'] = $existing->staff_identifi ?? $this->hr_profile_model->create_code('staff_code');
		} else {
			// Call with increment = true
			$data['staff_code'] = $this->hr_profile_model->create_code('staff_code', true);
		}

		$data['funcData']        = ['staff_id' => $id];
		...
		log_message('debug', 'Disability Types Passed: ' . print_r($data['disability_types'], true));
		log_message('debug', 'Data passed to new_member.php: ' . print_r($data, true));
...
private function upload_proof_file($input_name, $id)
{
    if (!isset($_FILES[$input_name])) {
        log_message('error', "[$input_name][$id] - File input not found in \$_FILES.");
        return null;
    }

    if (!isset($_FILES[$input_name]['name'][$id])) {
        log_message('error', "[$input_name][$id] - No file selected (index not found).");
        return null;
    }

    $upload_error = $_FILES[$input_name]['error'][$id];
    if ($upload_error !== UPLOAD_ERR_OK) {
        ...
        log_message('error', "[$input_name][$id] - Upload error: " . ($error_messages[$upload_error] ?? 'Unknown error'));
        return null;
    }

    // Save to public uploads folder
    $path = FCPATH . 'hr_profile/uploads/investment_proofs/';
    if (!file_exists($path)) {
        if (!mkdir($path, 0777, true)) {
            log_message('error', "[$input_name][$id] - Failed to create upload directory: $path");
            return null;
        }
    }

    $tmp_name = $_FILES[$input_name]['tmp_name'][$id];
    $orig_name = basename($_FILES[$input_name]['name'][$id]);
    $file_name = time() . '_' . $orig_name;
    $destination = $path . $file_name;

    if (move_uploaded_file($tmp_name, $destination)) {
        $url = admin_url('hr_profile/view_investment_file/' . urlencode($file_name));

        log_message('debug', "[$input_name][$id] - File uploaded successfully: $url");
        return $url;
    }

    log_message('error', "[$input_name][$id] - Failed to move file to: $destination");
    return null;
}


public function view_investment_file($filename)
{
    $path = FCPATH . 'hr_profile/uploads/investment_proofs/' . $filename;
