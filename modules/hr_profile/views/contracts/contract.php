							...
							<div class="col-md-6">
								...
								<select name="staff" class="selectpicker" id="staff" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" data-live-search="true"> 
									...
									<?php foreach($staff as $s){ ?>
										<option value="<?php echo new_html_entity_decode($s['staffid']); ?>"  <?php if(isset($contracts) && $contracts->staff == $s['staffid'] ){echo 'selected';} ?>> <?php echo new_html_entity_decode($s['firstname'].' '.$s['lastname']); ?></option>                  
									<?php }?>
								</select>
							</div>
						</div>
								...
								<div class="form-group">
									...
									<select name="name_contract" class="selectpicker" id="name_contract" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>"> 
										...
										<?php foreach($contract_type as $c){ ?>
											<option value="<?php echo new_html_entity_decode($c['id_contracttype']); ?>" <?php if(isset($contracts) && $contracts->name_contract == $c['id_contracttype'] ){echo 'selected';} ?>><?php echo new_html_entity_decode($c['name_contracttype']); ?> </option>
										<?php }?>
									</select>
								</div>
							</div>
    ...
    ?>   
						...
						<div class="row">
							...
							<div class="col-md-6">
								...
								<select name="staff_delegate" class="selectpicker" data-live-search="true" id="staff_delegate" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>"> 
									<option value=""></option>                  
									<?php foreach($staff as $s){ ?>
										<option value="<?php echo new_html_entity_decode($s['staffid']); ?>"  <?php if(isset($contracts) && $contracts->staff_delegate == $s['staffid'] ){echo 'selected';} ?>> <?php echo new_html_entity_decode($s['firstname'].' '.$s['lastname']); ?></option>                  
									<?php }?>
								</select>
							</div>
						</div>
				...
				<div class="modal-footer">
					...
					<?php if(has_permission('hrm_contract', '', 'create') || has_permission('hrm_contract', '', 'edit')){ ?>
						<a href="#"class="btn btn-primary pull-right mright10 display-block add_goods_receipt" ><?php echo _l('submit'); ?></a>
					<?php } ?>
				</div>
