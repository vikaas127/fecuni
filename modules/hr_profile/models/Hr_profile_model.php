...
public function calculate_tax()
    {
        ...
        $ctc = $this->hrm_model->get_ctc($employee_id);
        $declaration = $this->hrm_model->get_latest_declaration($employee_id);
        ...
        $deductions = $this->hrm_model->get_deductions($declaration, $regime, $employee_id);
        $taxable_income = max(0, $ctc - 50000 - $deductions);

        $tax = $this->hrm_model->calculate_tax_by_regime($taxable_income, $regime);

        echo json_encode(['tax' => round($tax)]);
    }


// public function calculate_tax_both_regimes($employee_id, $financial_year)
// {
//     $CI =& get_instance();
//     $CI->load->database();
...
//     log_activity("TAX: Starting tax calculation for employee_id=$employee_id, financial_year=$financial_year");

//     // 1. Fetch declaration
//     $declaration = $CI->db->get_where($prefix . 'hr_investment_declarations', [
//         'employee_id' => $employee_id,
//         'financial_year' => $financial_year,
//     ])->row();

//     if (!$declaration) {
//         log_activity("TAX ERROR: No declaration found for employee_id=$employee_id, year=$financial_year");
...
//     log_activity("TAX: Declaration found - ID=$declaration_id, selected_regime=$tax_regime");

//     // 2. Age from birthday
//     $staff = $CI->db->get_where($prefix . 'staff', ['staffid' => $staff_id])->row();
//     if (!$staff) {
//         log_activity("TAX ERROR: No staff found with staffid=$staff_id");
//         return ['error' => 'No staff found.'];
//     }

//     $dob = new DateTime($staff->birthday);
//     $today = new DateTime();
//     $age = $dob->diff($today)->y;

//     log_activity("TAX: Staff DOB={$staff->birthday}, Age=$age");

//     // 3. Fetch latest contract
//     $CI->db->where('staff', $staff_id); // FIXED: was 'staff_id'
//     $CI->db->order_by('id_contract', 'DESC');
//     $contract = $CI->db->get($prefix . 'hr_staff_contract')->row();

//     if (!$contract) {
//         log_activity("TAX ERROR: No contract found for staffid=$staff_id");
//         return ['error' => 'No contract found'];
//     }

//     $ctc = $contract->ctc;
//     log_activity("TAX: Latest contract found - CTC=$ctc");

//     // 4. Fetch salary components
//     $details = $CI->db->get_where($prefix . 'hr_staff_contract_detail', [
//         'staff_contract_id' => $contract->id_contract
//     ])->result();
...
//         if ($detail->rel_type === 'st_4') {
//             $basic_salary = $detail->annual_value;
//         } elseif ($detail->rel_type === 'st_5') {
//             $hra_received = $detail->annual_value;
//         }
//     }

//     log_activity("TAX: Basic Salary=$basic_salary, HRA Received=$hra_received");

//     // 5. HRA Exemption
//     $hra_exemption = 0;
//     if ($declaration->declares_hra === 'Yes') {
//         $hra = $CI->db->get_where($prefix . 'hr_hra_details', ['declaration_id' => $declaration_id])->row();
//         if ($hra) {
//             $rent_paid = $hra->monthly_rent * 12;
//             $is_metro = $hra->is_metro;

//             $condition1 = $hra_received;
//             $condition2 = $is_metro ? 0.5 * $basic_salary : 0.4 * $basic_salary;
//             $condition3 = $rent_paid - (0.1 * $basic_salary);

//             $hra_exemption = max(0, min($condition1, $condition2, $condition3));

//             log_activity("TAX: HRA Exemption calculated: $hra_exemption (Metro=$is_metro, Rent=$rent_paid)");
//         }
//     }

//     // 6. Home Loan
//     $home_loan_interest = 0;
//     if ($declaration->declares_home_loan === 'Yes') {
//         $home_loan = $CI->db->get_where($prefix . 'hr_home_loan_details', ['declaration_id' => $declaration_id])->row();
//         if ($home_loan) {
//             $home_loan_interest = $home_loan->annual_interest;
//             log_activity("TAX: Home Loan Interest=$home_loan_interest");
//         }
//     }

//     // 7. Investment deductions
//     $investment_items = $CI->db->get_where($prefix . 'hr_investment_items', [
//         'declaration_id' => $declaration_id
//     ])->result();

//     $deductions = [];
//     foreach ($investment_items as $item) {
//         if (!isset($deductions[$item->section])) {
//             $deductions[$item->section] = 0;
//         }
//         $deductions[$item->section] += $item->declared_amount;
//     }

//     log_activity("TAX: Raw Deductions: " . json_encode($deductions));

//     // 8. Apply deduction caps
//     $total_deductions_old = 0;

//     $total_deductions_old += min($deductions['80C'] ?? 0, 150000);
//     $total_deductions_old += min($deductions['80D'] ?? 0, 25000);
//     $total_deductions_old += min($deductions['80CCD'] ?? 0, 50000);
//     $total_deductions_old += $deductions['80E'] ?? 0;
//     $total_deductions_old += $deductions['80G'] ?? 0;
//     $total_deductions_old += $deductions['80TTA'] ?? 0;
//     $total_deductions_old += $home_loan_interest;
//     $total_deductions_old += $hra_exemption;

//     log_activity("TAX: Total Deductions (Old Regime): $total_deductions_old");

//     // 9. Taxable income
//     $taxable_income_old = max(0, $ctc - $total_deductions_old);
//     $taxable_income_new = max(0, $ctc);

//     // 10. Compute tax
//     function compute_old_tax($income)
//     {
//         $tax = 0;
...
//     function compute_new_tax($income)
//     {
//         $tax = 0;
//         $slabs = [
//             [0, 300000, 0],
...
//             [900001, 1200000, 0.15],
//             [1200001, 1500000, 0.2],
//             [1500001, PHP_INT_MAX, 0.3],
//         ];

//         foreach ($slabs as [$min, $max, $rate]) {
//             if ($income > $min) {
//                 $tax += (min($income, $max) - $min) * $rate;
//             }
//         }
//         return $tax;
//     }

//     $old_tax = compute_old_tax($taxable_income_old);
//     $new_tax = compute_new_tax($taxable_income_new);

//     log_activity("TAX: Computed Taxes - Old: $old_tax, New: $new_tax");

//     return [
//         'old_regime_tax' => round($old_tax, 2),
//         'new_regime_tax' => round($new_tax, 2),
//         'selected_regime' => $tax_regime,
...
function calculate_tax_both_regimes($employee_id, $financial_year)
{
    $CI =& get_instance();
    $CI->load->database();
    $prefix = db_prefix();
    ...
    if (!$declaration) return ['error' => 'No declaration found.'];

    $staff = $CI->db->get_where($prefix . 'staff', ['staffid' => $employee_id])->row();
    if (!$staff) return ['error' => 'No staff found.'];

    $contract = $CI->db->order_by('id_contract', 'DESC')->get_where($prefix . 'hr_staff_contract', ['staff' => $employee_id])->row();
    if (!$contract) return ['error' => 'No contract found'];

    $details = $CI->db->get_where($prefix . 'hr_staff_contract_detail', [
        'staff_contract_id' => $contract->id_contract
    ])->result();

    $basic_salary = $hra_received = 0;
    foreach ($details as $detail) {
        if ($detail->rel_type === 'st_4') $basic_salary = $detail->annual_value;
        elseif ($detail->rel_type === 'st_5') $hra_received = $detail->annual_value;
    }

    // --- HRA Exemption Calculation ---
   $hra_exemption = 0;
$rent_paid = 0;
$is_metro_flag = 0; // Default non-metro

$hra_entries = $CI->db->get_where($prefix . 'hr_hra_details', ['declaration_id' => $declaration->id])->result();

foreach ($hra_entries as $entry) {
    $monthly_rent = (float)$entry->monthly_rent;
    $from = new DateTime($entry->rental_from);
    $to = new DateTime($entry->rental_to);

    // Calculate number of months inclusive
    $months = ($to->format('Y') - $from->format('Y')) * 12 + ($to->format('n') - $from->format('n')) + 1;

    $rent_paid += $monthly_rent * $months;

    // Set metro flag to true if any of the entries is marked metro
    if ($entry->is_metro) {
        $is_metro_flag = 1;
    }
}
...
$condition1 = $hra_received;
$condition2 = $is_metro_flag ? 0.5 * $basic_salary : 0.4 * $basic_salary;
$condition3 = $rent_paid - (0.1 * $basic_salary);
...
$hra_exemption = max(0, min($condition1, $condition2, $condition3));
    ...
    $home_loan_interest = 0;
    if ($declaration->declares_home_loan === 'Yes') {
        $home_loan = $CI->db->get_where($prefix . 'hr_home_loan_details', ['declaration_id' => $declaration->id])->row();
        if ($home_loan) $home_loan_interest = $home_loan->annual_interest;
    }
    ...
    $deductions = [];
    foreach ($investment_items as $item) {
        $deductions[$item->section] = ($deductions[$item->section] ?? 0) + $item->declared_amount;
    }
    ...
    $total_deductions_old =
        min($deductions['80C'] ?? 0, 150000) +
        min($deductions['80D'] ?? 0, 25000) +
        min($deductions['80CCD'] ?? 0, 50000) +
        ($deductions['80E'] ?? 0) +
        ($deductions['80G'] ?? 0) +
        ($deductions['80TTA'] ?? 0) +
        $home_loan_interest +
    ...
    $taxable_income_old = max(0, $ctc - $total_deductions_old - $standard_deduction);
    $taxable_income_new = max(0, $ctc - $standard_deduction);

    // --- Tax Calculation Helper ---
    function compute_tax_breakdown($income, $slabs, $apply_87a = false)
    {
        $tax = 0;
        $breakdown = [];
        ...
        foreach ($slabs as [$min, $max, $rate]) {
            if ($income > $min) {
                $slab_amount = min($income, $max) - $min;
                $slab_tax = $slab_amount * $rate;
                $breakdown[] = [
                    'slab' => "₹" . number_format($min + 1) . " – ₹" . number_format($max),
                    'amount' => round($slab_amount),
                    'rate' => $rate * 100,
                    'tax' => round($slab_tax),
                ];
                $tax += $slab_tax;
        ...
        // Rebate under 87A
        if ($apply_87a && $income <= 500000 && $tax > 0) {
        ...
        return ['tax' => round($tax), 'breakdown' => $breakdown];
    }
    ...
    $old_result = compute_tax_breakdown($taxable_income_old, $old_slabs, true);
    $new_result = compute_tax_breakdown($taxable_income_new, $new_slabs, false);

    // Logging for debug
    log_message('debug', 'Tax Calculation for Emp: ' . $employee_id . ' | FY: ' . $financial_year . ' | Old Tax: ' . $old_result['tax'] . ' | New Tax: ' . $new_result['tax']);

    return [
        'old_regime_tax' => $old_result['tax'],
        'new_regime_tax' => $new_result['tax'],
        'hra_exemption' => $hra_exemption,
        'home_loan_interest' => $home_loan_interest,
        ...
        'taxable_income_old' => $taxable_income_old,
        'taxable_income_new' => $taxable_income_new,
        'old_regime_breakdown' => $old_result['breakdown'],
        'new_regime_breakdown' => $new_result['breakdown'],
        'raw_deductions' => $deductions,
		'rent_paid' => $rent_paid, 
        'selected_regime' => $declaration->tax_regime,
    ];
}
...
{
    log_message('debug', 'add_staff() called with: ' . print_r($data, true));

    // Remove fake remembered inputs
    unset($data['fakeusernameremembered'], $data['fakepasswordremembered']);

    // Check for duplicate email
    if ($this->db->where('email', $data['email'])->get(db_prefix() . 'staff')->row()) {
        log_message('error', 'Email already exists: ' . $data['email']);
        die('Email already exists');
    }

    // Handle admin
    $data['admin'] = (is_admin() && isset($data['administrator'])) ? 1 : 0;
    ...
    // Birthday / Identity Dates
    $data['birthday'] = !empty($data['birthday']) ? to_sql_date($data['birthday']) : null;
    $data['days_for_identity'] = !empty($data['days_for_identity']) ? to_sql_date($data['days_for_identity']) : null;
    ...
    if (!$staffid) {
        log_message('error', 'Failed to insert staff');
        return false;
    }

    log_message('debug', 'Inserted staff ID: ' . $staffid);

    
    

    // Staff media & misc
    $slug = trim($data['firstname'] . ' ' . $data['lastname']) ?: 'unknown-' . $staffid;
    $this->db->where('staffid', $staffid)->update(db_prefix() . 'staff', ['media_path_slug' => slug_it($slug)]);
    ...
    foreach ($this->db->get()->result_array() as $announcement) {
...
public function update_staff($data, $id)
{
    if (isset($data['fakeusernameremembered'])) unset($data['fakeusernameremembered']);
    if (isset($data['fakepasswordremembered'])) unset($data['fakepasswordremembered']);
    ...
    if (is_admin()) {
        if (isset($data['administrator'])) {
            ...
            if ($id != get_staff_user_id()) {
                if ($id == 1) return ['cant_remove_main_admin' => true];
            } else return ['cant_remove_yourself_from_admin' => true];
            $data['admin'] = 0;
        }
    }
    ...
    $data['two_factor_auth_enabled'] = isset($data['two_factor_auth_enabled']) ? 1 : 0;
    $data['is_not_staff'] = isset($data['is_not_staff']) ? 1 : 0;
    if (isset($data['admin']) && $data['admin'] == 1) $data['is_not_staff'] = 0;
    ...
    // ----------------- Staff Department Handling ------------------
    $this->load->model('departments_model');
  $staff_departments = $this->departments_model->get_staff_departments($id);

	// Assume $department holds the selected department ID (not an array)
	$department = $this->input->post('departments'); // name="departments" in the form

	if (!empty($staff_departments)) {
		// If no department is selected, delete all
    if (empty($department)) {
        $this->db->where('staffid', $id)->delete(db_prefix() . 'staff_departments');
    } else {
        foreach ($staff_departments as $staff_department) {
            if ($staff_department['departmentid'] != $department) {
                $this->db->where('staffid', $id)->where('departmentid', $staff_department['departmentid'])->delete(db_prefix() . 'staff_departments');
	...
	if (!empty($department)) {
		$this->db->where('staffid', $id)->where('departmentid', $department);
		if (!$this->db->get(db_prefix() . 'staff_departments')->row()) {
        $this->db->insert(db_prefix() . 'staff_departments', [
            'staffid' => $id,
            'departmentid' => $department
        ]);
...
public function add_edit_investments($data, $staffid)
{
    $this->db->trans_begin();

    $declaration_id = isset($data['declaration_id']) ? $data['declaration_id'] : null;
    ...
    // STEP 1: Insert or Update main declaration
    if (!$declaration_id) {
        $declaration = [
            'employee_id' => $data['staffid'],
            'declares_hra' => $data['declares_hra'],
            'declares_home_loan' => $data['declares_home_loan'],
			'financial_year' => $financial_year, 
        ];
        $this->db->insert(db_prefix() . 'hr_investment_declarations', $declaration);
        $declaration_id = $this->db->insert_id();
    } else {
        $update = [
            'declares_hra' => $data['declares_hra'],
            'declares_home_loan' => $data['declares_home_loan'],
        ...
        $this->db->update(db_prefix() . 'hr_investment_declarations', $update);
    }

    // STEP 2: HRA
    // if (!empty($data['hra'])) {
    //     foreach ($data['hra'] as $hra) {
    //         $hra['declaration_id'] = $declaration_id;
    //         if (!empty($hra['id'])) {
    //             $this->db->where('id', $hra['id']);
    //             unset($hra['id']);
    //             $this->db->update(db_prefix() . 'hr_hra_details', $hra);
    //         } else {
    //             $this->db->insert(db_prefix() . 'hr_hra_details', $hra);
    //         }
    //     }
    // }
	// STEP 2: HRA
	if (!empty($data['hra'])) {
		foreach ($data['hra'] as $hra) {
			...
			// ✅ Fix: Convert 'YYYY-MM' to 'YYYY-MM-01'
			if (!empty($hra['rental_from']) && strlen($hra['rental_from']) === 7) {
				$hra['rental_from'] .= '-01';
			}
			if (!empty($hra['rental_to']) && strlen($hra['rental_to']) === 7) {
				$hra['rental_to'] .= '-01';
			}
    ...
    if (!empty($data['loans'])) {
        foreach ($data['loans'] as $loan) {
            $loan['declaration_id'] = $declaration_id;
            if (!empty($loan['id'])) {
                $this->db->where('id', $loan['id']);
                unset($loan['id']);
                $this->db->update(db_prefix() . 'hr_home_loan_details', $loan);
            } else {
                $this->db->insert(db_prefix() . 'hr_home_loan_details', $loan);
            }
        }
    }
    ...
    if (!empty($data['investments'])) {
        foreach ($data['investments'] as $inv) {
            $inv['declaration_id'] = $declaration_id;
            if (!empty($inv['id'])) {
                $this->db->where('id', $inv['id']);
                unset($inv['id']);
                $this->db->update(db_prefix() . 'hr_investment_items', $inv);
            } else {
                $this->db->insert(db_prefix() . 'hr_investment_items', $inv);
            }
        }
    }

    // STEP 5: Handle Removed Rows
    if (!empty($data['removed_hra_ids'])) {
        $this->db->where_in('id', explode(',', $data['removed_hra_ids']));
        $this->db->delete(db_prefix() . 'hr_hra_details');
    }

    if (!empty($data['removed_loan_ids'])) {
        $this->db->where_in('id', explode(',', $data['removed_loan_ids']));
        $this->db->delete(db_prefix() . 'hr_home_loan_details');
    }

    if (!empty($data['removed_investment_ids'])) {
        $this->db->where_in('id', explode(',', $data['removed_investment_ids']));
        $this->db->delete(db_prefix() . 'hr_investment_items');
    }
	...
	public function get_latest_declaration_by_staff($staff_id) {
		$this->db->where('employee_id', $staff_id);
		$this->db->order_by('submitted_on', 'DESC');
		$this->db->limit(1);
		return $this->db->get(db_prefix() . 'hr_investment_declarations')->row();
	}
	public function get_hra_details($declaration_id) {
    $results = $this->db->where('declaration_id', $declaration_id)
                        ->get(db_prefix() . 'hr_hra_details')
                        ->result_array();

    foreach ($results as &$row) {
        if (!empty($row['rental_from'])) {
            $row['rental_from'] = date('Y-m', strtotime($row['rental_from']));
        }
        if (!empty($row['rental_to'])) {
            $row['rental_to'] = date('Y-m', strtotime($row['rental_to']));
        }
    }
	...
	public function get_loan_details($declaration_id) {
		return $this->db->where('declaration_id', $declaration_id)
						->get(db_prefix() . 'hr_home_loan_details')->result_array();
	}
	public function get_investment_items($declaration_id) {
		return $this->db->where('declaration_id', $declaration_id)
						->get(db_prefix() . 'hr_investment_items')->result_array();
	}
	...
	public function add_contract($data){
	    ...
	    // Check if the staff already has a contract
		// $this->db->where('staff', $data['staff']);
		// $existing_contract = $this->db->get(db_prefix() . 'hr_staff_contract')->row();

		// if ($existing_contract) {
		// 	// Option 1: Return false or custom error array
		// 	return ['error' => 'Contract already exists for this staff member.'];
		// }
...
public function create_code($rel_type)
{
    ...
    switch ($rel_type) {
        case 'staff_code':
            $prefix_str = get_hr_profile_option('staff_code_prefix');
            $option_key = 'staff_code_number';
            break;

        case 'position_code':
            $prefix_str = get_hr_profile_option('job_position_prefix');
            $option_key = 'job_position_number';
            break;

        case 'staff_contract_code':
            $prefix_str = get_hr_profile_option('contract_code_prefix');
            $option_key = 'contract_code_number';
            break;
    ...
    // ✅ Fetch current number from options table
    $next_number = (int) get_hr_profile_option($option_key);
    if ($next_number == 0) {
        $next_number = 1; // fallback default
    }

    // Build the padded code
    $code = $prefix_str . str_pad($next_number, 5, '0', STR_PAD_LEFT);

    // ✅ Check if the code already exists in the DB (only for staff_code)
    if ($rel_type === 'staff_code') {
        while ($this->db->where('staff_identifi', $code)->count_all_results(db_prefix() . 'staff') > 0) {
            $next_number++;
            $code = $prefix_str . str_pad($next_number, 5, '0', STR_PAD_LEFT);
        }
    }
    ...
    update_option($option_key, $next_number + 1);
