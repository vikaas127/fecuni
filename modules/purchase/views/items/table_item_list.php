        ...
        if ($aColumns[$i] == db_prefix().'items.id') {
         ...
         if($aColumns[$i] == 'commodity_code') {
            ...
            if (has_permission('purchase_items', '', 'edit') || is_admin()) {
                $code .= ' | <a href="#" onclick="edit_commodity_item(this); return false;"  data-commodity_id="'.$aRow[db_prefix().'items.id'].'" data-description="'.$aRow['description'].'" data-unit_id="'.$aRow['unit_id'].'" data-commodity_code="'.$aRow['commodity_code'].'" data-commodity_barcode="'.$aRow['commodity_barcode'].'" data-rate="'.$aRow['rate'].'" data-group_id="'.$aRow['group_id'].'" data-tax="'.$aRow['tax'].'" data-tax2="'.$aRow['tax2'].'"  data-sku_code="'.$aRow['sku_code'].'" data-sku_name="'.$aRow['sku_name'].'"     data-currency="'.$aRow['currency'].'"
    data-ex_work_price="'.$aRow['ex_work_price'].'"
    data-sheet_type_id="'.$aRow['sheet_type_id'].'"
    data-material_type_id="'.$aRow['material_type_id'].'"
    data-color="'.$aRow['color'].'"
    data-finish_type_id="'.$aRow['finish_type_id'].'"    
    data-size_id="'.$aRow['size_id'].'"
    data-Length="'.$aRow['Length'].'"
    data-Thickness="'.$aRow['Thickness'].'"

data-purchase_price="'.$aRow['purchase_price'].'" >' . _l('edit') . '</a>';
            }
