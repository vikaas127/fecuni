        ...
        if ($insert_id) {
            ...
            if(count($order_detail) > 0){
                foreach($order_detail as $key => $rqd){ 
                    ...
                    $dt_data['unit_id'] = isset($rqd['unit_id']) ? $rqd['unit_id'] : null;
                    $dt_data['unit_price'] = $rqd['unit_price'];
                    $dt_data['ex_work_price'] = $rqd['ex_work_price'];
    ...
    $html .= '<table class="table">
      ...
      $html .=  '<table class="table purorder-item">
        <thead>
          <tr>
            ...
            <th class="thead-dark" style="width: 15%;" align="right">'._l('recieved_quantity').'</th>';
      ...
      foreach($pur_order_detail as $row){
        ...
        $html .= '<tr nobr="true" class="sortable">
            ...
            <td style="width: 15%;" align="right">'.app_format_number($row['wh_quantity_received'],'').' '. $unit_name.'</td>';
...
// Fetch all conversion rates (you already have this)
public function get_conversion_rate()
{
    return $this->db->get(db_prefix().'_pconversion')->result_array();
}

// Save or update
public function save_conversion_rate($data)
{
    if(isset($data['id']) && $data['id'] != ''){
        $this->db->where('id', $data['id']);
        $this->db->update(db_prefix().'_pconversion', $data);
        return $data['id'];
    } else {
        $this->db->insert(db_prefix().'_pconversion', $data);
...
{
    $this->db->where('id', $id);
    return $this->db->delete(db_prefix().'_pconversion');
}
    ...
    */
    ...
    public function get_statement($vendor_id, $from, $to)
    {
        ...
        WHERE ' . $sqlDatePayments . ' AND ' . db_prefix() . 'pur_invoices.vendor = ' . $this->db->escape_str($vendor_id) . ' '.$sql_pay.' AND ' . db_prefix() . 'pur_invoices.approval_status = 2
        ...
        FROM ' . db_prefix() . 'pur_invoice_payment
        JOIN ' . db_prefix() . 'pur_invoices ON ' . db_prefix() . 'pur_invoices.id = ' . db_prefix() . 'pur_invoice_payment.pur_invoice
        WHERE ' . $sqlDatePayments . ' AND ' . db_prefix() . 'pur_invoices.vendor = ' . $this->db->escape_str($vendor_id).' '.$sql_amount_paid .' AND ' . db_prefix() . 'pur_invoices.approval_status = 2' )
            ->row()->amount_paid;
    ...
    public function get_item_v2($id = '')
    {
        ...
        $this->db->select($rateCurrencyColumns . '' . db_prefix() . 'items.id as itemid,rate,
            ...
            CONCAT(commodity_code,"_",description) as code_description,long_description,group_id,' . db_prefix() . 'items_groups.name as group_name,unit,'.db_prefix().'ware_unit_type.unit_name as unit_name,ex_work_price, purchase_price, unit_id, guarantee');
...
public function get_usd_to_inr_rate() {
    $this->db->select('to_currency_rate');
    $this->db->from(db_prefix() . 'currency_rates'); // tblcurrency_rates
    $this->db->where('id', 1);
    $query = $this->db->get();

    if ($query->num_rows() > 0) {
        return $query->row()->to_currency_rate;
    }
    return null; // fallback if not found
}

     public function get_total_pvalue_in_inr()
{
    $row = $this->db->select_sum('p_value')->get(db_prefix().'_pconversion')->row();
    return isset($row->p_value) ? (float) $row->p_value : 0.00;
}

public function create_purchase_order_row_template($name = '', $item_name = '', $item_description = '', $quantity = '', $unit_name = '',$ex_work_price='',          $unit_price = '', $taxname = '',  $item_code = '', $unit_id = '', $tax_rate = '', $total_money = '', $discount = '', $discount_money = '', $total = '', $into_money = '', $tax_id = '', $tax_value = '', $item_key = '',$is_edit = false, $currency_rate = 1, $to_currency = '') {
    ...
    $this->load->model('invoice_items_model');
        ...
        $name_unit_name = 'unit_name';
        $name_quantity = 'quantity';
        $name_ex_work_price='ex_work_price';
        $name_unit_price = 'unit_price';
        $name_tax_id_select = 'tax_select';
        $name_tax_id = 'tax_id';
        $name_total = 'total';
        ...
        if ($name == '') {
            ...
            $name_unit_name = '[unit_name]';
            $name_quantity = $name . '[quantity]';
            $name_ex_work_price=$name.'[ex_work_price]';
            $name_unit_price = $name . '[unit_price]';
            $name_tax_id_select = $name . '[tax_select][]';
            $name_tax_id = $name . '[tax_id]';
            $name_total = $name . '[total]';
        ...
        $row .= '<td class="">' . render_textarea($name_item_name, '', $item_name, ['rows' => 2, 'placeholder' => _l('pur_item_name'), 'readonly' => true] ) . '</td>';
         $row .= '<input type="hidden" name="' . $name . '[currency]" value="' . $currency . '">';
        $row .= '<td class="">' . render_textarea($name_item_description, '', $item_description, ['rows' => 2, 'placeholder' => _l('item_description')] ) . '</td>';


        // Fetch extra value in base currency
        $extra_value = $this->purchase_model->get_total_pvalue_in_inr();

        // Convert extra_value if currency is different from base
        $base_currency = get_base_currency()->id; // assuming this returns base currency ID
        $extra_value_in_currency = $extra_value;

        if ($to_currency != 0 && $to_currency != $base_currency) {
            $extra_value_in_currency = $extra_value * $currency_rate;
        }

        // Ex-work price input
        $row .= '<td class="rate">' 
            . render_input($name_ex_work_price, '', $ex_work_price, 'number', 
                array_merge($array_rate_attr, ['class' => 'ex_work_price']), // <-- add class
                [], 'no-margin', $text_right_class);

        // Show original price in base currency if different
        if ($ex_work_price != '') {
            $original_price = ($currency_rate > 0) ? round(($ex_work_price / $currency_rate), 2) : 0;

            if ($to_currency != 0 && $to_currency != $base_currency) {
                $row .= render_input('original_price', '', app_format_money($original_price, get_base_currency()), 'text', [
                    'data-toggle' => 'tooltip',
                    'data-placement' => 'top',
                    'title' => _l('original_price'),
                    'disabled' => true
                ], [], 'no-margin', 'input-transparent text-right pur_input_none');
            }

            $row .= '<input class="hide" name="og_price" disabled="true" value="' . $original_price . '">';
        }

        // Final unit price = ex_work_price + extra_value (converted)
        $unit_price = (float)$ex_work_price + $extra_value_in_currency;

        // Render unit price input
        $row .= '<td class="">' 
            . render_input($name_unit_price, '', $unit_price, 'number', 
                array_merge($array_rate_attr, ['class' => 'unit_price']), // <-- add class
                [], 'no-margin', $text_right_class) 
            . '</td>';
...
public function get_filtered_items($vendor, $Length, $Thickness, $color_id, $finish_type_id,$sheet_type_id)
{
    $this->db->select('i.*');
    $this->db->from(db_prefix().'items i');
    $this->db->join(db_prefix().'pur_vendor_items iv', 'iv.items = i.id', 'inner');
    $this->db->where('iv.vendor', $vendor);
    ...
    $query = $this->db->get();
    return $query->result_array(); // flat list
}
...
public function get_item_Lengths() {
    ...
    $this->db->from(db_prefix().'items');
    $query = $this->db->get();
    return array_column($query->result_array(), 'Length');
}

// Thicknesses
public function get_item_Thicknesses() {
    $this->db->distinct();
    $this->db->select('Thickness');
    $this->db->from(db_prefix().'items');
    $query = $this->db->get();
    return array_column($query->result_array(), 'Thickness');
}

public function get_item_development_lengths() {
    $this->db->distinct();
    $this->db->select('development_length');
    $this->db->from(db_prefix().'items');
    $query = $this->db->get();
    return array_column($query->result_array(), 'development_length');
}

// Colors
public function get_colors() {
    $this->db->select('c.color_id, c.color_name');
    $this->db->from(db_prefix().'items i');
    $this->db->join(db_prefix().'ware_color c','i.color = c.color_id','left');
...
public function get_finishes() {
    $this->db->select('f.finish_type_id, f.finish_name');
    $this->db->from(db_prefix().'items i');
    $this->db->join(db_prefix().'mrp_finish f','i.finish_type_id = f.finish_type_id','left');
    $this->db->group_by('f.finish_type_id');
    return $this->db->get()->result_array();
}

public function get_sheets() {
    $this->db->select('f.sheet_type_id, f.sheet_name');
    $this->db->from(db_prefix().'items i');
    $this->db->join(db_prefix().'wh_sheet f','i.sheet_type_id = f.sheet_type_id','left');
    $this->db->group_by('f.sheet_type_id');
    return $this->db->get()->result_array();
}


public function get_profiles() {
    $this->db->select('f.profile_type_id, f.profile_name');
    $this->db->from(db_prefix().'items i');
    $this->db->join(db_prefix().'mrp_profile f','i.profile_type_id = f.profile_type_id','left');
    $this->db->group_by('f.profile_type_id');
    return $this->db->get()->result_array();
}
