...
$(document).ready(function(){
    ...
    function updateTotal(){
        ...
        $('#conversionTable tbody tr').each(function(){
            let val = parseFloat($(this).find('.p_value').val()) || 0;
            total += val;
        });
    ...
    // Save existing or new row
    $(document).on('click', '.saveRow', function(){
        let row = $(this).closest('tr');
        let id = row.data('id');
        let p_name = row.find('.p_name').val().trim();
        let p_value = row.find('.p_value').val();
        ...
        $.post("<?php echo admin_url('purchase/save_conversion_rate'); ?>", 
        {id: id, p_name: p_name, p_value: p_value}, 
        function(response){
            ...
            if(res.status == 'success'){
                ...
                if(!id){ // new row added
                    // change button text to 'Save' and add delete button
                    row.find('.saveRow').removeClass('btn-primary').addClass('btn-success').text('Save');
                    row.find('td:last').append('<button class="btn btn-sm btn-danger deleteRow">Delete</button>');

                    // append new empty row for next addition
                    ...
                    $('#conversionTable tbody').append(newRow);
                }
    ...
    $(document).on('click', '.deleteRow', function(){
        ...
        if(!id){ // just remove unsaved row
            row.remove();
            updateTotal();
            return;
        }

        $.post("<?php echo admin_url('purchase/delete_conversion_rate'); ?>", {id: id}, function(response){
            let res = JSON.parse(response);
            if(res.status == 'success'){
                row.remove();
                updateTotal();
    ...
    // recalc total when value changes
    $(document).on('input', '.p_value', function(){
        updateTotal();
    });
