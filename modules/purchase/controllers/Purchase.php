...
class purchase extends AdminController
{
  ...
  if (is_admin()) {
        ...
        $data['vendor_categories'] = $this->purchase_model->get_vendor_category();
        $data['conversion_rate'] = $this->purchase_model->get_conversion_rate();
    ...
    // Save/Add row
public function save_conversion_rate()
{
    if (!is_admin() && !has_permission('purchase_settings', '', 'edit')) {
        access_denied('purchase');
    }
    ...
    if(empty($data['p_name']) || $data['p_value'] === '') {
        echo json_encode(['status' => 'error', 'message' => 'Name and value required']);
        return;
    }

    $saved_id = $this->purchase_model->save_conversion_rate(array_merge($data, ['id' => $id]));
    echo json_encode(['status' => 'success', 'id' => $saved_id]);
}

// Delete row
public function delete_conversion_rate()
{
    if (!is_admin() && !has_permission('purchase_settings', '', 'delete')) {
        access_denied('purchase');
    }

    $id = $this->input->post('id');
    if($this->purchase_model->delete_conversion_rate($id)){
     ...
     */
     ...
     public function pur_batch_payment_modal() {
        ...
        $this->load->view('admin/purchase/invoices/batch_payment_modal', $data);
    }
    ...
    public function coppy_pur_request_for_po($pur_request, $vendor = ''){
        ...
        if(count($pur_request_detail) > 0){
            ...
            foreach($pur_request_detail as $key => $item){
                ...
                $list_item .= $this->purchase_model->create_purchase_order_row_template('newitems[' . $index_quote . ']',  $item_name,'', $item['quantity'], $unit_name, $item['ex_work_price'], $item['unit_price'], $taxname, $item['item_code'], $item['unit_id'], $item['tax_rate'],  $item['total'], '', '', $item['total'], $item['into_money'], $item['tax'], $item['tax_value'], $index_quote, true, $currency_rate, $to_currency);
            }
        }
    ...
    public function coppy_pur_estimate($pur_estimate_id){
        ...
        if(count($pur_estimate_detail) > 0){
            ...
            foreach($pur_estimate_detail as $key => $item){
                ...
                $list_item .= $this->purchase_model->create_purchase_order_row_template('newitems[' . $index . ']',  $item_name, '', $item['quantity'], $unit_name, $item['ex_work_price'],$item['unit_price'], $taxname, $item['item_code'], $item['unit_id'], $item['tax_rate'],  $item['total_money'], $item['discount_%'], $item['discount_money'], $item['total'], $item['into_money'], $item['tax'], $item['tax_value'], $index, true, $currency_rate, $to_currency);
            }
        }
    ...
    public function pur_order($id = ''){
        ...
        if ($id == '') {
            ...
            if (count($data['pur_order_detail']) > 0) { 
                ...
                foreach ($data['pur_order_detail'] as $order_detail) { 
                    ...
                    $pur_order_row_template .= $this->purchase_model->create_purchase_order_row_template('items[' . $index_order . ']',  $item_name, $order_detail['description'], $order_detail['quantity'], $unit_name, $order_detail['ex_work_price'], $order_detail['unit_price'], $taxname, $order_detail['item_code'], $order_detail['unit_id'], $order_detail['tax_rate'],  $order_detail['total_money'], $order_detail['discount_%'], $order_detail['discount_money'], $order_detail['total'], $order_detail['into_money'], $order_detail['tax'], $order_detail['tax_value'], $order_detail['id'], true, $currency_rate, $to_currency);
                }
            }
        }
        ...
        $usd_to_inr_currency_rate = $this->purchase_model->get_usd_to_inr_rate();
        $data['usd_to_inr_currency_rate'] = $usd_to_inr_currency_rate;
        ...
        $data['units'] = $this->purchase_model->get_units();
        $data['Lengths'] = $this->purchase_model->get_item_Lengths();
        $data['Thicknesses'] = $this->purchase_model->get_item_Thicknesses();

        $data['colors'] = $this->purchase_model->get_colors();

        $data['finishes'] = $this->purchase_model->get_finishes();
        $data['sheets'] = $this->purchase_model->get_sheets();
...
public function filter_items_by_vendor()
{
    $vendor = $this->input->post('vendor');
    $Length = $this->input->post('Length');       
    $Thickness = $this->input->post('Thickness'); 
    $color_id = $this->input->post('color_id');
    $finish_type_id = $this->input->post('finish_type_id');
    $sheet_type_id = $this->input->post('sheet_type_id');


    $this->load->model('purchase_model');
    $items = $this->purchase_model->get_filtered_items($vendor, $Length, $Thickness, $color_id, $finish_type_id,$sheet_type_id);

    $html = '';
    if(!empty($items)){
        foreach($items as $item){
            $html .= '<option value="'.$item['id'].'" data-subtext="('.$item['purchase_price'].')">'.$item['description'].'</option>';
        }
    } else {
        $html = '<option value="">'. _l('no_items_found') .'</option>';
    }

    echo json_encode(['option_html'=>$html]);
}
...
/**
    ...
    public function pur_order_change($ct){
        ...
        if(count($pur_order_detail) > 0){
            ...
            foreach($pur_order_detail as $key => $item){
                ...
                $list_item .= $this->purchase_model->create_purchase_invoice_row_template('newitems[' . $index . ']',  $item_name, '', $item['quantity'], $unit_name, $item['ex_work_price'],$item['unit_price'], $taxname, $item['item_code'], $item['unit_id'], $item['tax_rate'],  $item['total_money'], $item['discount_%'], $item['discount_money'], $item['total'], $item['into_money'], $item['tax'], $item['tax_value'], $index, true, $currency_rate, $to_currency);
            }
        }
                ...
                if (!empty($tmpFilePath) && $tmpFilePath != '') {
                    ...
                    if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                        ...
                        $writer_header = array(
                            ...
                            _l('ex_work_price')                     =>'string',
                            _l('sheet_type')                     =>'string',
                        ...
                        for ($row = 1; $row < count($data); $row++) {
                            ...
                            $value_ex_work_price   = isset($data[$row][10]) ? $data[$row][10] : '';
                            $value_sheet_type_id        = isset($data[$row][11]) ? $data[$row][11] : '';
                            $value_material_type_id  = isset($data[$row][12]) ? $data[$row][12] : '';
                            $value_color_id             = isset($data[$row][13]) ? $data[$row][13] : '';
                            $value_finish_type_id    = isset($data[$row][14]) ? $data[$row][14] : '';
                            $value_Length            = isset($data[$row][15]) ? $data[$row][15] : '';
                            $value_Thickness         = isset($data[$row][16]) ? $data[$row][16] : '';
                            $value_Width             = isset($data[$row][17]) ? $data[$row][17] : '';


                            $value_rate            = isset($data[$row][18]) ? $data[$row][18] : '';
                            $value_tax            = isset($data[$row][19]) ? $data[$row][19] : '';
                            $value_tax2            = isset($data[$row][20]) ? $data[$row][20] : '';
                            ...
                            if (is_null($value_commodity_group) != true && ($value_commodity_group != '0') && $value_commodity_group != '') {
                            	...
                            	if (is_null($value_material_type_id) != true && ($value_material_type_id != '0') && $value_material_type_id != '') {
									/*case input id*/
									if (is_numeric($value_material_type_id)) {

										$this->db->where('material_type_id', $value_material_type_id);
										...
										if ($material_type_id_value == 0) {
											...
											$flag_id_material_type_id = $value_material_type_id;
										}

									} else {
										/*case input name*/
										$this->db->like(db_prefix() . 'mrp_material.material_name', $value_material_type_id);
                                	...
                                	if (is_null($value_color_id) != true && ($value_color_id != '0') && $value_color_id != '') {
									/*case input id*/
									if (is_numeric($value_color_id)) {

										$this->db->where('color_id', $value_color_id);
										...
										if ($color_id_value == 0) {
											...
											/*get id color_id*/
											$flag_id_color_id = $value_color_id;
										}

									} else {
										/*case input name*/
										$this->db->like(db_prefix() . 'ware_color.color_code', $value_color_id);
								...
								if (is_null($value_finish_type_id) != true && ($value_finish_type_id != '0') && $value_finish_type_id != '') {
									/*case input id*/
									if (is_numeric($value_finish_type_id)) {

										$this->db->where('finish_type_id', $value_finish_type_id);
										...
										if ($finish_type_id_value == 0) {
											...
											$flag_id_finish_type_id = $value_finish_type_id;
										}

									} else {
										/*case input name*/
										$this->db->like(db_prefix() . 'mrp_finish.finish_name', $value_finish_type_id);
                                ...
                                if (is_null($value_sheet_type_id) != true && ($value_sheet_type_id != '0') && $value_sheet_type_id != '') {
                            if (is_numeric($value_sheet_type_id)) {

                                $this->db->where('sheet_type_id', $value_sheet_type_id);
                                $sheet_type_id_value = $this->db->count_all_results(db_prefix() . 'wh_sheet');

                                if ($sheet_type_id_value == 0) {
                                    $string_error .= _l('sheet_type') . _l('does_not_exist');
                                    $flag2 = 1;
                                } else {
                                    $flag_id_sheet_type_id = $value_sheet_type_id;
                                }

                            } else {
                                /* case input name */
                                $this->db->like(db_prefix() . 'wh_sheet.sheet_name', $value_sheet_type_id);
                                $sheet_type_id_value = $this->db->get(db_prefix() . 'wh_sheet')->result_array();

                                if (count($sheet_type_id_value) == 0) {
                                    $string_error .= _l('sheet_type') . _l('does_not_exist');
                                    $flag2 = 1;
                                } else {
                                    $flag_id_sheet_type_id = $sheet_type_id_value[0]['sheet_type_id'];
                                }
                            }
                        }
                            ...
                            if(($flag == 1) || $flag2 == 1 ){
                                ...
                                $writer->writeSheetRow('Sheet1', [
                                    ...
                                    $value_ex_work_price,
                                    $value_sheet_type_id,
                                    $value_material_type_id,
                                    $value_color_id,
                                    $value_finish_type_id,
                                    $value_Length,
                                    $value_Thickness,
                                    $value_Width,
                                    $value_commodity_group,
                            ...
                            if($flag == 0 && $flag2 == 0){
                                ...
                                $rd['ex_work_price']                         = reformat_currency_pur($value_ex_work_price);
                                $rd['sheet_type_id']     = isset($flag_id_sheet_type_id) ? $flag_id_sheet_type_id : '';
                                ...
                                $rd['Length']    = $value_Length;
                                $rd['Thickness'] = $value_Thickness;
                                $rd['Width']     = $value_Width;

									$material_name = '';
									if (!empty($value_material_type_id)) {
										...
										if (!$material) {
											$this->db->select('material_name');
											$this->db->where('material_code', $value_material_type_id);
											$material = $this->db->get(db_prefix() . 'mrp_material')->row();
										}
									...
									$sheet_name = '';
									if (!empty($value_sheet_type_id)) {
										$this->db->select('sheet_name');
										$this->db->where('sheet_type_id', $value_sheet_type_id);
										$sheet = $this->db->get(db_prefix() . 'wh_sheet')->row();

										if (!$material) {
											$this->db->select('sheet_name');
											$this->db->where('sheet_code', $value_sheet_type_id);
											$sheet = $this->db->get(db_prefix() . 'wh_sheet')->row();
										}

										$sheet_name = $sheet ? $sheet->sheet_name : '';
									}
									

									// Finish
									$finish_name = '';
									if (!empty($value_finish_type_id)) {
										...
										if (!$finish) {
											$this->db->select('finish_name');
											$this->db->where('finish_code', $value_finish_type_id);
											$finish = $this->db->get(db_prefix() . 'mrp_finish')->row();
										}
									...
									if (!empty($value_color_id)) {
										...
										if (!$color) {
											$this->db->select('color_name');
											$this->db->where('color_code', $value_color_id);
											$color = $this->db->get(db_prefix() . 'ware_color')->row();
										}
									...
									// Final Description
									$rd['description'] = $material_name . ' | ' . $sheet_name . ' | ' . $color_name . ' | ' . $finish_name .' | ' .$value_Length . ' | ' . $value_Thickness  ;
    ...
    public function get_item_by_id($id, $currency_rate = 1)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $item->extra_value = $this->purchase_model->get_total_pvalue_in_inr();
    ...
    public function get_purchase_order_row_template(){
        ...
        $quantity = $this->input->post('quantity');
        $unit_name = $this->input->post('unit_name');
        $ex_work_price = $this->input->post('ex_work_price');
        ...
        echo $this->purchase_model->create_purchase_order_row_template($name, $item_name, $item_description, $quantity, $unit_name,$ex_work_price, $unit_price, $taxname, $item_code, $unit_id, $tax_rate, '', $discount, '', '', '', '', '', $item_key, false, $currency_rate, $to_currency);
    }
