...
<div class="panel_s accounting-template estimate">
    ...
    <label for="customer_group"><?php echo _l('customer_group'); ?></label>
    <input type="text" id="customer_group" class="form-control" 
           value="<?php echo $customer_group; ?>" readonly>
</div>
               ...
               ?>
                ...
                <div class="col-md-6">
                  ...
                  $value = '';
                  if (isset($estimate)) {
                      $value = _d($estimate->delivery_date);
                  } else {
                   //   if (get_option('estimate_due_after') != 0) {
                    //      $value = _d(date('Y-m-d', strtotime('+' . get_option('estimate_due_after') . ' DAY', strtotime(date('Y-m-d')))));
                    //  }
                  }
                  echo render_date_input('delivery_date', 'estimate_add_edit_deliverydate', $value); ?>
                    </div>
                    <div class="col-md-6">
    <div class="form-group" style="position: relative;">
        <label for="Tolerance"><?php echo _l('Tolerance'); ?></label>
        <input type="text" name="Tolerance" id="tolerance"
               class="form-control pr-4"
               value="<?php echo isset($tolerance) ? html_escape($tolerance) : ''; ?>"
               oninput="this.value=this.value.replace(/[^0-9.]/g,'');"
               <?php echo isset($disabled_edit) ? $disabled_edit : ''; ?>>
        <span style="
            position: absolute;
            right: 15px;
            top: 32px;
...
<div class="modal fade" id="priceCalcModal" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-lg" role="document" style="width:95%;max-width:1200px">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"><?php echo _l('Price Calculator'); ?></h4>
        <button type="button" class="close" data-dismiss="modal">
          <span>&times;</span>
        </button>
      </div>
      <div class="modal-body" style="padding:0">
        <iframe id="priceCalcFrame" src="" style="width:100%;height:75vh;border:0;"></iframe>
      </div>
    </div>
  </div>
</div>
...
<script>
(function(){
  var baseUrl = '<?= admin_url('estimates/price_calculator'); ?>'; // direct to controller method
  var $btn = document.getElementById('btn-open-price-calculator');
  var $frame = document.getElementById('priceCalcFrame');

  if ($btn) {
    $btn.addEventListener('click', function(){
      $frame.src = baseUrl; // add ?color=..&finish=.. if prefill needed
