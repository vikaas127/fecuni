...
//                     'unit'             => $item['unit'],
//                 ]);
//     log_message('error', 'Itemable Insert Data: ' . json_encode($insert_data));
...
function add_new_sales_item_post($item, $rel_id, $rel_type)
{
    ...
    $table_name = db_prefix() . 'itemable';
    log_message('error', 'Inserting into table: ' . $table_name);

    // Log columns in the table
    $columns = $CI->db->list_fields($table_name);
    log_message('error', 'Columns in ' . $table_name . ': ' . json_encode($columns));
    ...
    $CI->db->insert($table_name, $insert_data);

    log_message('error', 'Itemable Insert Query: ' . $CI->db->last_query());

    $id = $CI->db->insert_id();

    log_message('error', 'Itemable Insert ID: ' . $id);
