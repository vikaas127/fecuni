...
class Clients extends AdminController
{
    ...
    public function import()
    {
        ...
        // Log the total number of clients before import
        $totalClientsBefore = total_rows(db_prefix() . 'clients');
    
        log_message('info', 'Client import started. Total clients before import: ' . $totalClientsBefore);    
        ...
        if ($this->input->post() && isset($_FILES['file_csv']['name']) && $_FILES['file_csv']['name'] != '') {
            ...
            if (!$this->import->perform()) {
                log_message('error', 'Customer import failed.');
            }
    
            $data['total_rows_post'] = $this->import->totalRows();
    
            if (!$this->import->isSimulation()) {
                $imported = $this->import->totalImported();
                if ($imported === 0) {
                    log_message('error', 'No customers were imported. Possible errors in CSV or database constraints.');
                }
                set_alert('success', _l('import_total_imported', $imported));
            }
    
            // Log the total number of clients after import
            $totalClientsAfter = total_rows(db_prefix() . 'clients');
            log_message('info', 'Client import completed. Total clients after import: ' . $totalClientsAfter);
        } else {
    ...
    public function sheet_discount()
{
    if (!is_admin()) {
        access_denied('Customer Sheet Discount');
    }

    // Fetch groups and sheets from the database
    $data['groups'] = $this->db->get(db_prefix() . 'customers_groups')->result_array();
    $data['sheets'] = $this->db->get(db_prefix() . 'wh_sheet')->result_array();

    // Log for debugging
    log_message('debug', 'Groups: ' . print_r($data['groups'], true));
    log_message('debug', 'Sheets: ' . print_r($data['sheets'], true));

    // Handle AJAX request for DataTable
    if ($this->input->is_ajax_request()) {
        $this->app->get_table_data('client_sheet_discount');
        return;
    }

    $data['title'] = _l('customer_sheet_discount');
    $this->load->view('admin/clients/sheet_discounts_manage', $data);


}
// Add / Edit Client Sheet Discount
public function client_sheet_discount()
{
    if (!is_admin()) {
        access_denied('Customer Sheet Discount');
    }
    ...
    if (!$data) {
        set_alert('danger', 'No data posted');
        redirect(admin_url('clients/sheet_discount'));
    }
    ...
    // Add new
    if (empty($data['id'])) {
        $id = $this->clients_model->add_client_sheet_discount($data);
        if ($id) {
            set_alert('success', 'Sheet discount added successfully');
        } else {
            set_alert('danger', 'Failed to add sheet discount');
        }
    }
    // Edit existing
    else {
        $success = $this->clients_model->edit_client_sheet_discount($data);
        if ($success) {
            set_alert('success', 'Sheet discount updated successfully');
        } else {
            set_alert('danger', 'Failed to update sheet discount');
        }
    }
...
public function get_used_sheets_by_group($group_id, $current_id = null)
{
    // Ensure admin access
    if (!is_admin()) {
        echo json_encode([]);
    ...
    $this->db->from(db_prefix() . '_sheet_customer_grp_discount');
    ...
    $query = $this->db->get();
    $sheet_ids = array_column($query->result_array(), 'sheet_id');

    // Return JSON array of sheet IDs
    echo json_encode($sheet_ids);
}

// Delete Client Sheet Discount
public function delete_sheet_customer_grp_discount($id)
{
    if (!is_admin()) {
        access_denied('Customer Sheet Discount');
    }
    ...
    $success = $this->clients_model->delete_client_sheet_discount($id);

    if ($success) {
        set_alert('success', 'Sheet discount deleted successfully');
    } else {
        set_alert('danger', 'Failed to delete sheet discount');
    }
