...
class Leads extends AdminController
{
    ...
    public function convert_to_customer()
    {
        ...
        log_message('debug', 'Starting convert_to_customer function');
        ...
        if ($this->input->post()) {
            ...
            if ($id) {
                log_message('debug', 'Customer created successfully with ID: ' . $id);
            } else {
                log_message('error', 'Failed to create customer from lead ID: ' . $data['leadid']);
            }
            if ($id) {
                ...
                log_message('debug', 'Redirecting to: ' . admin_url('clients/client/' . $id));
