    ...
    public function filter_items_flat()
{
    ...
    $this->load->model('invoice_items_model');
    $items = $this->invoice_items_model->get_filtered_items_for_estimate_flat($Length, $Thickness, $color_id, $finish_type_id, $sheet_type_id);
    ...
    public function get_proposal_data_ajax($id, $to_return = false)
{
    log_message('debug', 'Called get_proposal_data_ajax with ID: ' . $id);

    if (staff_cant('view', 'proposals') && staff_cant('view_own', 'proposals') && get_option('allow_staff_view_proposals_assigned') == 0) {
        log_message('error', 'Access denied for proposal ID: ' . $id);
        echo _l('access_denied');
        die;
    }

    $proposal = $this->proposals_model->get($id, [], true);
    log_message('debug', 'Fetched proposal: ' . print_r($proposal, true));

    if (!$proposal || !user_can_view_proposal($id)) {
        log_message('error', 'Proposal not found or access denied for proposal ID: ' . $id);
        echo _l('proposal_not_found');
        die;
    }
    ...
    $this->app_mail_template->set_rel_id($proposal->id);
    $data = prepare_mail_preview_data('proposal_send_to_customer', $proposal->email);
    log_message('debug', 'Prepared mail preview data: ' . print_r($data, true));
    ...
    $extra_merge_fields = $this->app_merge_fields->get_flat('proposals', 'other', '{email_signature}');
    log_message('debug', 'Extra merge fields: ' . print_r($extra_merge_fields, true));

    $merge_fields = array_merge($merge_fields, $extra_merge_fields);

    $data['proposal_statuses']     = $this->proposals_model->get_statuses();
    log_message('debug', 'Proposal statuses: ' . print_r($data['proposal_statuses'], true));

    $data['members']               = $this->staff_model->get('', ['active' => 1]);
    log_message('debug', 'Active members: ' . print_r($data['members'], true));

    $data['proposal_merge_fields'] = $merge_fields;
    $data['proposal']              = $proposal;
    $data['totalNotes']            = total_rows(db_prefix() . 'notes', ['rel_id' => $id, 'rel_type' => 'proposal']);
    log_message('debug', 'Total notes for proposal ID ' . $id . ': ' . $data['totalNotes']);

    if ($to_return == false) {
        log_message('debug', 'Loading proposal view (not returning HTML)');
        $this->load->view('admin/proposals/proposals_preview_template', $data);
    } else {
        log_message('debug', 'Returning proposal view as HTML string');
        return $this->load->view('admin/proposals/proposals_preview_template', $data, true);
    }
}
