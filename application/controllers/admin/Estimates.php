...
class Estimates extends AdminController
{
    ...
    public function estimate($id = '')
    {
        ...
        $group = $this->client_groups_model->get_customer_group_name($customer_id);
        $data['customer_group'] = $group ? $group->group_name : '';
        ...
        // Preload all items for dropdown
        $all_items = $this->invoice_items_model->get_all_items();
        $items_grouped = [];
        foreach ($all_items as $item) {
            $items_grouped[$item['group_id']][] = $item;
        }

        $data['items'] = $items_grouped;
        $data['ajaxItems'] = true; // keep AJAX search enabled for large datasets
        ...
        $data['development_lengths'] = $this->purchase_model->get_item_development_lengths();
        ...
        $data['profiles'] = $this->purchase_model->get_profiles();
...
public function get_customer_group($customer_id = null)
{
    if (!$customer_id) {
        show_404();
    }
...
public function filter_items_flat()
{
    $Length = $this->input->post('Length');
    $Thickness = $this->input->post('Thickness');
    $development_length=$this->input->post('development_length');
    ...
    $this->load->model('invoice_items_model');
    $items = $this->invoice_items_model->get_filtered_items_for_estimate_flat($Length, $Thickness,$development_length, $color_id, $finish_type_id, $sheet_type_id,$profile_type_id);

    echo json_encode($items);
}


public function get_customer_group_ajax($id)
{
    if (!is_staff_logged_in()) {
        log_message('error', 'Unauthorized access attempt to get_customer_group_ajax with ID: ' . $id);
        show_error('Forbidden', 403);
    }

    $this->load->model('clients_model');

    log_message('debug', 'Fetching customer groups for customer ID: ' . $id);

    $groups = $this->clients_model->get_customer_groups($id);

    if (empty($groups)) {
        log_message('info', 'No customer groups found for customer ID: ' . $id);
    } else {
        log_message('info', 'Customer groups found for customer ID: ' . $id . ' → ' . json_encode($groups));
    }
    ...
    public function price_calculator()
{
    ...
    // Optional prefill from GET
    $data['color']     = $this->estimates_model->get_colors();
    $data['finish']    = $this->estimates_model->get_finish();
    $data['length']    = [2438, 3048]; 
    $data['thickness'] = [0.6, 0.8, 1.0, 1.2];
    $data['customer'] = $this->Clients_model->get();

    $data['embed'] = true; // so we can hide large headers if needed

    // Load your calculator view directly
    $this->load->view('admin/estimates/NormalCalculator', $data);
}
