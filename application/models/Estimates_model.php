    ...
    public function get_colors()
{
    ...
    return $this->db->select('color_id,color_name')
        ->from('tblware_color')
        ->order_by('color_name', 'ASC')
        ->get()->result_array();   // => [ ['id'=>1,'name'=>'Silver'], ... ]
}
 public function get_finish(){
    // Adjust column names if your table uses different ones
    return $this->db->select('finish_code,finish_name')
        ->from('tblmrp_finish')
        ->order_by('finish_name', 'ASC')
        ->get()->result_array();   // => [ ['id'=>1,'name'=>'Silver'], ... ]
}


   public function get_filtered_items($Length, $Thickness,$development_length, $color_id, $finish_type_id, $sheet_type_id,$profile_type_id)
    {
        $this->db->select('i.*');
...
public function get_active()
    {
        return $this->db->order_by('id', 'DESC')->limit(1)->get('price_settings')->row_array();
    }

    public function upsert($data)
    {
        $current = $this->get_active();
        if ($current) {
            $this->db->where('id', $current['id'])->update('price_settings', $data);
            return $current['id'];
        }
        $this->db->insert('price_settings', $data);
